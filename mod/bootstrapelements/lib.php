 ...
 * @copyright  2014 Birmingham City University <michael.grant@bcu.ac.uk>. 2024 modified for Moodle 4.1+ by Dominik Cuber <office@cubesites.de>
...
function bootstrapelements_get_coursemodule_info($coursemodule) {
    ...
    if ($bootstrapelements = $DB->get_record('bootstrapelements', array('id' => $coursemodule->instance),
        ...
        if (!$bootstrapelements->name || $bootstrapelements->name == 'bootstrapelements') {
            // Set the name to the value of the 'title' column
            $bootstrapelements->name = $bootstrapelements->title;
            // Update the 'name' column in the database with the title value
            $DB->set_field('bootstrapelements', 'name', $bootstrapelements->title, array('id' => $bootstrapelements->id));
        }
        ...
        switch($bootstrapelements->bootstraptype) {
            case 0:
                $modal_name = 'bootstrapelements' . $bootstrapelements->id; // Modal name
                $toggle_name = 'bootstrapelements' . $bootstrapelements->id; // Toggle name
                $info->content = bootstrapelements_modal_outline($modal_name, $bootstrapelements->title,
                        format_module_intro('bootstrapelements', $bootstrapelements, $coursemodule->id, false),
                        $bootstrapelements->bootstrapicon).
                        bootstrapelements_modal_button($modal_name, $bootstrapelements->title,
                        $bootstrapelements->bootstrapicon);
                break;

            case 1:
                $toggle_name = 'bootstrapelements' . $bootstrapelements->id; // Toggle name
                $info->content = bootstrapelements_toggle_outline($toggle_name, $bootstrapelements->title,
                        format_module_intro('bootstrapelements', $bootstrapelements, $coursemodule->id, false),
                        $bootstrapelements->bootstrapicon);
                break;
        ...
        // Update the link (href) to include 'bootstrapelements' and the ID
        $info->content = str_replace('href="#' . $bootstrapelements->name . '"', 'href="#bootstrapelements' . $bootstrapelements->id . '"', $info->content);
        // Update the modal and toggle fields to use the same ID
        $info->content = str_replace('id="' . $bootstrapelements->name . '"', 'id="bootstrapelements' . $bootstrapelements->id . '"', $info->content);
