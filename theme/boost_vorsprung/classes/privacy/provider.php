...
/**
 * Privacy Subsystem implementation for theme_boost_vorsprung.
 *
...
namespace theme_boost_vorsprung\privacy;
...
/**
 * The boost_vorsprung theme stores a user preference data.
 *
 * @copyright  2018 Andrew Nicols <andrew@nicols.co.uk>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class provider implements
    ...
    public static function export_user_preferences(int $userid) {
        ...
        if (isset($draweropenindexpref)) {
            $preferencestring = get_string('privacy:drawerindexclosed', 'theme_boost_vorsprung');
            if ($draweropenindexpref == 1) {
                $preferencestring = get_string('privacy:drawerindexopen', 'theme_boost_vorsprung');
            }
            \core_privacy\local\request\writer::export_user_preference(
                'theme_boost_vorsprung',
        ...
        if (isset($draweropenblockpref)) {
            $preferencestring = get_string('privacy:drawerblockclosed', 'theme_boost_vorsprung');
            if ($draweropenblockpref == 1) {
                $preferencestring = get_string('privacy:drawerblockopen', 'theme_boost_vorsprung');
            }
            \core_privacy\local\request\writer::export_user_preference(
                'theme_boost_vorsprung',
