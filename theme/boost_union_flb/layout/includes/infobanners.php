...
for ($i = 1; $i <= THEME_boost_union_flb_SETTING_INFOBANNER_COUNT; $i++) {
    ...
    if (theme_boost_union_flb_infobanner_is_shown_on_page($i)) {
        ...
        if ($dismissible == true) {
            ...
            user_preference_allow_ajax_update('theme_boost_union_flb_infobanner'.$i.'_dismissed', PARAM_BOOL);
        }
...
usort($infobanners, 'theme_boost_union_flb_compare_order');
...
if ($dismissibleamdneeded == true) {
    $PAGE->requires->js_call_amd('theme_boost_union_flb/infobanner', 'init');
}
