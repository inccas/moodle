...
function theme_boost_union_flb_get_main_scss_content($theme) {
    ...
    // This way, Boost Union FlB will ship the same SCSS code as Boost Union itself.
    $scss = theme_boost_union_get_main_scss_content(\core\output\theme_config::load('boost_union'));

    // And add Boost Union FlB's main SCSS file to the stack.
    $scss .= file_get_contents($CFG->dirroot . '/theme/boost_union_flb/scss/post.scss');
...
function theme_boost_union_flb_get_pre_scss($theme) {
    ...
    // If such a fault happens, the Boost Union FlB administrator can switch the inheritance to 'Duplicate'.
    // This way, we will add the pre SCSS code with the explicit use of the Boost Union configuration to the stack.
    $inheritanceconfig = get_config('theme_boost_union_flb', 'prescssinheritance');
    if ($inheritanceconfig == THEME_BOOST_UNION_FLB_SETTING_INHERITANCE_DUPLICATE) {
        $scss .= theme_boost_union_get_pre_scss(\core\output\theme_config::load('boost_union'));
    }

    // And add Boost Union FlB's pre SCSS file to the stack.
    $scss .= file_get_contents($CFG->dirroot . '/theme/boost_union_flb/scss/pre.scss');
...
function theme_boost_union_flb_get_extra_scss($theme) {
    ...
    $inheritanceconfig = get_config('theme_boost_union_flb', 'extrascssinheritance');
    if ($inheritanceconfig == THEME_BOOST_UNION_FLB_SETTING_INHERITANCE_DUPLICATE) {
...
/**
 * Callback function for theme_boost_union to allow Boost Union FlB to add cards to the Boost Union settings overview page.
 * This function is expected to return an array of arrays containing values with the keys 'label', 'desc', 'btn' and 'url'.
 *
 * @return array
 */
function theme_boost_union_flb_extend_busettingsoverview() {

    $cards[] = [
        'label' => get_string('pluginname', 'theme_boost_union_flb'),
        'desc' => get_string('settingsoverview_buc_desc', 'theme_boost_union_flb'),
        'btn' => 'primary',
        'url' => new \core\url('/admin/settings.php', ['section' => 'theme_boost_union_flb']),
    ];
 ...
 */
function theme_boost_union_flb_alter_css_urls(&$urls) {
    global $CFG;
