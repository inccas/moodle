...
namespace theme_boost_union_flb\output;
use context_system;
use moodle_url;
...
class core_renderer extends \theme_boost\output\core_renderer {
    ...
    public function favicon() {
            ...
            if ($flavour != null) {
                ...
                if ($flavour->look_favicon != null) {
                    ...
                    $flavourfaviconurl = moodle_url::make_pluginfile_url(
                            context_system::instance()->id, 'theme_boost_union_flb', 'flavours_look_favicon', $flavour->id,
        ...
        if (!during_initial_install()) {
            $logo = get_config('theme_boost_union_flb', 'favicon');
        }
        if (empty($logo)) {
        ...
        return moodle_url::make_pluginfile_url(context_system::instance()->id, 'theme_boost_union_flb', 'favicon', '64x64/',
    ...
    public function get_logo_url($maxwidth = null, $maxheight = 200) {
            ...
            if ($flavour != null) {
                ...
                if ($flavour->look_logo != null) {
                    ...
                    $flavourlogourl = moodle_url::make_pluginfile_url(
                            context_system::instance()->id, 'theme_boost_union_flb', 'flavours_look_logo', $flavour->id,
        ...
        $logo = get_config('theme_boost_union_flb', 'logo');
        if (empty($logo)) {
            return false;
        }
        ...
        if (in_array($logoextension, ['svg', 'svgz'])) {
            // The theme_boost_union_flb_pluginfile() function will look for a filepath and will try to extract the size from that.
            // Thus, we cannot drop the filepath from the URL completely.
            // But we can add a path without an 'x' in it which will then be interpreted by theme_boost_union_flb_pluginfile()
        ...
        // Use $CFG->themerev to prevent browser caching when the file changes.
        return moodle_url::make_pluginfile_url(context_system::instance()->id, 'theme_boost_union_flb', 'logo', $filepath,
                theme_get_revision(), $logo);
    }
    ...
    public function get_compact_logo_url($maxwidth = 300, $maxheight = 300) {
        ...
        if ($hasflavourlogo != null) {
            ...
            if ($flavour != null) {
                ...
                if ($flavour->look_logocompact != null) {
                    ...
                    $flavourlogourl = moodle_url::make_pluginfile_url(
                            context_system::instance()->id, 'theme_boost_union_flb', 'flavours_look_logocompact', $flavour->id,
        ...
        $logo = get_config('theme_boost_union_flb', 'logocompact');
        if (empty($logo)) {
            return false;
        }
        ...
        // Use $CFG->themerev to prevent browser caching when the file changes.
        return moodle_url::make_pluginfile_url(context_system::instance()->id, 'theme_boost_union_flb', 'logocompact', $filepath,
                theme_get_revision(), $logo);
    }
    ...
    public function body_attributes($additionalclasses = array()) {
        ...
        if ($this->page->pagelayout != 'login') {
            if (!empty(get_config('theme_boost_union_flb', 'backgroundimage'))) {
        ...
        if ($this->page->pagelayout == 'login') {
            ...
            $loginimageclass = theme_boost_union_flb_get_random_loginbackgroundimage_class();
    ...
    public function full_header() {
        ...
        if ($this->page->pagelayout == 'course' && (get_config('theme_boost_union_flb', 'courseheaderimageenabled')
            ...
            $header->courseheaderimageheight = get_config('theme_boost_union_flb', 'courseheaderimageheight');
            // Additionally, get the course header image position.
            $header->courseheaderimageposition = get_config('theme_boost_union_flb', 'courseheaderimageposition');
            // Additionally, get the template context attributes for the course header image layout.
            $courseheaderimagelayout = get_config('theme_boost_union_flb', 'courseheaderimagelayout');
            switch($courseheaderimagelayout) {
                case THEME_boost_union_flb_SETTING_COURSEIMAGELAYOUT_HEADINGABOVE:
                    $header->courseheaderimagelayoutheadingabove = true;
                    $header->courseheaderimagelayoutstackedclass = '';
                    break;
                case THEME_boost_union_flb_SETTING_COURSEIMAGELAYOUT_STACKEDDARK:
                    $header->courseheaderimagelayoutheadingabove = false;
                    $header->courseheaderimagelayoutstackedclass = 'dark';
                    break;
                case THEME_boost_union_flb_SETTING_COURSEIMAGELAYOUT_STACKEDLIGHT:
                    $header->courseheaderimagelayoutheadingabove = false;
    ...
    public function navbar(): string {
        $newnav = new \theme_boost_union_flb\boostnavbar($this->page);
    ...
    public function block(\block_contents $bc, $region) {
        ...
        $regions = theme_boost_union_flb_get_additional_regions();
        $regioncapname = array_search($region, $regions);
        if (!empty($regioncapname) && $context->hascontrols) {
            $context->hascontrols = has_capability('theme/boost_union_flb:editregion'.$regioncapname, $this->page->context);
        }
