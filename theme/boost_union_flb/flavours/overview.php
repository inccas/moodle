...
$PAGE->set_url(new moodle_url('/theme/boost_union_flb/flavours/overview.php'));
...
if ($action && $flavourid) {
    // Sort 'up' action.
    if ($action == 'up') {
        $currentflavour = $DB->get_record('theme_boost_union_flb_flavours', array('id' => $flavourid));
        $prevflavour = $DB->get_record('theme_boost_union_flb_flavours', array('sort' => $currentflavour->sort - 1));
        if ($prevflavour) {
            // The sorting is done with two DB statements which should have a monolithic effect,
            // so we use a transaction.
            $transaction = $DB->start_delegated_transaction();
            $DB->set_field('theme_boost_union_flb_flavours', 'sort', $prevflavour->sort,
                    array('id' => $currentflavour->id));
            $DB->set_field('theme_boost_union_flb_flavours', 'sort', $currentflavour->sort,
            ...
            cache_helper::purge_by_event('theme_boost_union_flb_flavours_resorted');
        }

        // Sort 'down' action.
    } else if ($action = "down") {
        $currentflavour = $DB->get_record('theme_boost_union_flb_flavours', array('id' => $flavourid));
        $nextflavour = $DB->get_record('theme_boost_union_flb_flavours', array('sort' => $currentflavour->sort + 1));
        if ($nextflavour) {
            // The sorting is done with two DB statements which should have a monolithic effect,
            // so we use a transaction.
            $transaction = $DB->start_delegated_transaction();
            $DB->set_field('theme_boost_union_flb_flavours', 'sort', $nextflavour->sort,
                    array('id' => $currentflavour->id));
            $DB->set_field('theme_boost_union_flb_flavours', 'sort', $currentflavour->sort,
...
// Access checks.
admin_externalpage_setup('theme_boost_union_flb_flavours');

// Further prepare the page.
$PAGE->set_title(theme_boost_union_flb_get_externaladminpage_title(get_string('flavoursflavours', 'theme_boost_union_flb')));
$PAGE->set_heading(theme_boost_union_flb_get_externaladminpage_heading());

// Build flavours table.
$table = new \theme_boost_union_flb\flavours_overview_table();
$table->define_baseurl($PAGE->url);

// Start page output.
echo $OUTPUT->header();
echo $OUTPUT->heading(get_string('configtitleflavours', 'theme_boost_union_flb'));

// Show flavours description.
echo get_string('flavoursoverview_desc', 'theme_boost_union_flb');

// Prepare 'Create flavours' button.
$createbutton = $OUTPUT->box_start();
$createbutton .= $OUTPUT->single_button(
        new \moodle_url('/theme/boost_union_flb/flavours/edit.php', ['action' => 'create']),
        get_string('flavourscreateflavour', 'theme_boost_union_flb'), 'get');
