...
class eventobservers {
    ...
    public static function cohort_deleted(\core\event\base $event) {
        ...
        if (theme_boost_union_flb_flavour_exists_for_cohort($event->objectid)) {
            ...
            \cache_helper::purge_by_event('theme_boost_union_flb_cohort_deleted');
        }
    }
    ...
    public static function cohort_member_added(\core\event\base $event) {
        ...
        if (theme_boost_union_flb_flavour_exists_for_cohort($event->objectid)) {
            ...
            // after adding him to a cohort. This flag is checked in theme_boost_union_flb_get_flavour_which_applies() where,
            ...
            set_user_preference('theme_boost_union_flb_flavours_purgesessioncache', true, $event->relateduserid);
        }
    }
    ...
    public static function cohort_member_removed(\core\event\base $event) {
        ...
        if (theme_boost_union_flb_flavour_exists_for_cohort($event->objectid)) {
            // Set a flag within a user preference for the affected user as the user might get other flavours which apply
            // after removing him from a cohort. This flag is checked in theme_boost_union_flb_get_flavour_which_applies() where,
            // if set, the flavours cache for the affected user is cleared.
            // This way, we avoid that the flavours cache is purged unnecessarily for all users.
            set_user_preference('theme_boost_union_flb_flavours_purgesessioncache', true, $event->relateduserid);
        }
