...
/**
 * Theme Boost Union VORsprung - Theme config
 *
...
// This move will save us from duplicating all lines from Boost Union's config.php into Boost Union VORsprung's config.php.
...
// Then, we require Boost Union VORsprung's locallib.php to make sure that it's always loaded.
require_once($CFG->dirroot . '/theme/boost_union_vorsprung/locallib.php');

// Next, we overwrite only the settings which differ between Boost Union and Boost Union VORsprung.
$THEME->name = 'boost_union_vorsprung';
$THEME->scss = function($theme) {
    return theme_boost_union_vorsprung_get_main_scss_content($theme);
};
$THEME->parents = ['boost_union', 'boost'];
$THEME->extrascsscallback = 'theme_boost_union_vorsprung_get_extra_scss';
$THEME->prescsscallback = 'theme_boost_union_vorsprung_get_pre_scss';
...
// Lastly, we replicate some settings from Boost Union at runtime into Boost Union VORsprung's settings.
...
// it would be quite ugly to duplicate all of these settings again to Boost Union VORsprung.
