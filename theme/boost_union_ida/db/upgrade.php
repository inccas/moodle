...
/**
 * Function to upgrade theme_boost_union_ida
 * @param int $oldversion the version we are upgrading from
 * @return bool result
 */
function xmldb_theme_boost_union_ida_upgrade($oldversion) {
    global $DB, $OUTPUT;
    ...
    if ($oldversion < 2022080916) {
        ...
        // Define table theme_boost_union_ida_flavours to be created.
        $table = new xmldb_table('theme_boost_union_ida_flavours');

        // Adding fields to table theme_boost_union_ida_flavours.
        ...
        // Adding keys to table theme_boost_union_ida_flavours.
        $table->add_key('primary', XMLDB_KEY_PRIMARY, ['id']);

        // Conditionally launch create table for theme_boost_union_ida_flavours.
        if (!$dbman->table_exists($table)) {
            $dbman->create_table($table);
        }

        // boost_union_ida savepoint reached.
        upgrade_plugin_savepoint(true, 2022080916, 'theme', 'boost_union_ida');
    }

    if ($oldversion < 2022080922) {

        // Start composing the notification to inform the admin.
        $message = html_writer::tag('p', get_string('upgradenotice_2022080922', 'theme_boost_union_ida'));
        ...
        foreach (['logo', 'logocompact'] as $setting) {
            ...
            if (!empty($logocore)) {
                ...
                if ($files) {
                    ...
                    $filerecord = array(
                            'component' => 'theme_boost_union_ida',
                            'filearea' => $setting,
                    );
                    ...
                    set_config($setting, '/'.$newfile->get_filename(), 'theme_boost_union_ida');
            ...
            if ($logocopied == true) {
                ...
                $message .= html_writer::tag('p', get_string('upgradenotice_2022080922_copied', 'theme_boost_union_ida',
                        get_string('upgradenotice_2022080922_'.$setting, 'theme_boost_union_ida')));

                // Otherwise, if no logo was copied.
            } else {
                // Add the corresponding note to the notification.
                $message .= html_writer::tag('p', get_string('upgradenotice_2022080922_notcopied', 'theme_boost_union_ida',
                        get_string('upgradenotice_2022080922_'.$setting, 'theme_boost_union_ida')));
            }
        }
        ...
        // boost_union_ida savepoint reached.
        upgrade_plugin_savepoint(true, 2022080922, 'theme', 'boost_union_ida');
    }
