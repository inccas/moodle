@theme @theme_boost_union_ida @theme_boost_union_ida_functionalitysettings @theme_boost_union_ida_functionalitysettings_courses
Feature: Configuring the theme_boost_union_ida plugin for the "Courses" tab on the "Functionality" page
  ...
  Scenario: Setting: Show hint for switched role - Enable the setting
    Given the following config values are set as admin:
      | config                   | value | plugin             |
      | showswitchedroleincourse | yes   | theme_boost_union_ida |
  ...
  Scenario: Setting: Show hint in hidden courses - Enable the setting
    Given the following config values are set as admin:
      | config               | value | plugin             |
      | showhintcoursehidden | yes   | theme_boost_union_ida |
    When I log in as "teacher1"
    And I am on "Course 1" course homepage
  ...
  Scenario: Setting: Show hint guest for access - Enable the setting
    Given the following config values are set as admin:
      | config                    | value | plugin             |
      | showhintcourseguestaccess | yes   | theme_boost_union_ida |
  ...
  Scenario: Setting: Show hint for self enrolment without enrolment key - Enable the setting
    Given the following config values are set as admin:
      | config                  | value | plugin             |
      | showhintcourseselfenrol | yes   | theme_boost_union_ida |
    When I log in as "teacher1"
    And I am on "Course 1" course homepage
