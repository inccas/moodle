@theme @theme_boost_union_ida @theme_boost_union_ida_looksettings @theme_boost_union_ida_looksettings_loginpage
Feature: Configuring the theme_boost_union_ida plugin for the "Login page" tab on the "Look" page
  ...
  Scenario: Setting: Login page background images - Upload one custom login background image
    ...
    And I upload "theme/boost_union_ida/tests/fixtures/login_bg1.jpg" file to "Login page background images" filemanager
  ...
  Scenario: Setting: Login page background images - Upload multiple custom login background image (and have one picked randomly)
    ...
    And I click on "Login page" "link" in the "#adminsettings .nav-tabs" "css_element"
    And I upload "theme/boost_union_ida/tests/fixtures/login_bg1.jpg" file to "Login page background images" filemanager
    And I upload "theme/boost_union_ida/tests/fixtures/login_bg2.jpg" file to "Login page background images" filemanager
    And I upload "theme/boost_union_ida/tests/fixtures/login_bg3.jpg" file to "Login page background images" filemanager
    And I press "Save changes"
    And I log out
    And I click on "Log in" "link" in the ".logininfo" "css_element"
    Then the "class" attribute of "body" "css_element" should contain "path-login"
  ...
  Scenario Outline: Setting: Login form position
    Given the following config values are set as admin:
      | config            | value     | plugin            |
      | loginformposition | <setting> | theme_boost_union_ida |
    When I am on site homepage
    And I click on "Log in" "link" in the ".logininfo" "css_element"
  ...
  Scenario Outline: Setting: Login form transparency
    Given the following config values are set as admin:
      | config                | value     | plugin            |
      | loginformtransparency | <setting> | theme_boost_union_ida |
    When I am on site homepage
    And I click on "Log in" "link" in the ".logininfo" "css_element"
