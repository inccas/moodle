...
function theme_boost_union_ida_get_main_scss_content($theme) {
    ...
    // This way, Boost Union IDA will ship the same SCSS code as Boost Union itself.
...
function theme_boost_union_ida_get_pre_scss($theme) {
    ...
    // If such a fault happens, the Boost Union IDA administrator can switch the inheritance to 'Duplicate'.
    // This way, we will add the pre SCSS code with the explicit use of the Boost Union configuration to the stack.
    $inheritanceconfig = get_config('theme_boost_union_ida', 'prescssinheritance');
    if ($inheritanceconfig == THEME_BOOST_UNION_IDA_SETTING_INHERITANCE_DUPLICATE) {
...
function theme_boost_union_ida_get_extra_scss($theme) {
    ...
    $inheritanceconfig = get_config('theme_boost_union_ida', 'extrascssinheritance');
    if ($inheritanceconfig == THEME_BOOST_UNION_IDA_SETTING_INHERITANCE_DUPLICATE) {
...
/**
 * Callback function for theme_boost_union to allow Boost Union IDA to add cards to the Boost Union settings overview page.
 * This function is expected to return an array of arrays containing values with the keys 'label', 'desc', 'btn' and 'url'.
 *
 * @return array
 */
function theme_boost_union_ida_extend_busettingsoverview() {

    $cards[] = [
        'label' => get_string('pluginname', 'theme_boost_union_ida'),
        'desc' => get_string('settingsoverview_buc_desc', 'theme_boost_union_ida'),
        'btn' => 'primary',
        'url' => new \core\url('/admin/settings.php', ['section' => 'theme_boost_union_ida']),
    ];
 ...
 */
function theme_boost_union_ida_alter_css_urls(&$urls) {
    global $CFG;
