...
switch ($config->{'tilefrontpageposition'}) {
    case THEME_boost_union_ida_SETTING_ADVERTISEMENTTILES_FRONTPAGEPOSITION_BEFORE:
        $templatecontext['advtilespositionbefore'] = true;
        $templatecontext['advtilespositionafter'] = false;
        break;
    case THEME_boost_union_ida_SETTING_ADVERTISEMENTTILES_FRONTPAGEPOSITION_AFTER:
...
for ($i = 1; $i <= THEME_boost_union_ida_SETTING_ADVERTISEMENTTILES_COUNT; $i++) {
    ...
    if ($config->{'tile'.$i.'enabled'} == THEME_BOOST_UNION_FLB_SETTING_SELECT_YES) {
        ...
        if ($config->{'tile'.$i.'linktarget'} == THEME_boost_union_ida_SETTING_LINKTARGET_NEWTAB) {
        ...
        $bgimage = theme_boost_union_ida_get_urloftilebackgroundimage($i);
...
usort($advertisementtiles, 'theme_boost_union_ida_compare_order');
