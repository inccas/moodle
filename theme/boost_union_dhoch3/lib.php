...
function theme_boost_union_dhoch3_get_main_scss_content($theme) {
    ...
    // This way, Boost Union DHoch3 will ship the same SCSS code as Boost Union itself.
    $scss = theme_boost_union_get_main_scss_content(\core\output\theme_config::load('boost_union'));

    // And add Boost Union DHoch3's main SCSS file to the stack.
    $scss .= file_get_contents($CFG->dirroot . '/theme/boost_union_dhoch3/scss/post.scss');
...
function theme_boost_union_dhoch3_get_pre_scss($theme) {
    ...
    // If such a fault happens, the Boost Union DHoch3 administrator can switch the inheritance to 'Duplicate'.
    // This way, we will add the pre SCSS code with the explicit use of the Boost Union configuration to the stack.
    $inheritanceconfig = get_config('theme_boost_union_dhoch3', 'prescssinheritance');
    if ($inheritanceconfig == THEME_BOOST_UNION_DHOCH3_SETTING_INHERITANCE_DUPLICATE) {
        $scss .= theme_boost_union_get_pre_scss(\core\output\theme_config::load('boost_union'));
    }

    // And add Boost Union DHoch3's pre SCSS file to the stack.
    $scss .= file_get_contents($CFG->dirroot . '/theme/boost_union_dhoch3/scss/pre.scss');
...
function theme_boost_union_dhoch3_get_extra_scss($theme) {
    ...
    $inheritanceconfig = get_config('theme_boost_union_dhoch3', 'extrascssinheritance');
    if ($inheritanceconfig == THEME_BOOST_UNION_DHOCH3_SETTING_INHERITANCE_DUPLICATE) {
...
/**
 * Callback function for theme_boost_union to allow Boost Union DHoch3 to add cards to the Boost Union settings overview page.
 * This function is expected to return an array of arrays containing values with the keys 'label', 'desc', 'btn' and 'url'.
 *
 * @return array
 */
function theme_boost_union_dhoch3_extend_busettingsoverview() {

    $cards[] = [
        'label' => get_string('pluginname', 'theme_boost_union_dhoch3'),
        'desc' => get_string('settingsoverview_buc_desc', 'theme_boost_union_dhoch3'),
        'btn' => 'primary',
        'url' => new \core\url('/admin/settings.php', ['section' => 'theme_boost_union_dhoch3']),
    ];
 ...
 */
function theme_boost_union_dhoch3_alter_css_urls(&$urls) {
    global $CFG;
