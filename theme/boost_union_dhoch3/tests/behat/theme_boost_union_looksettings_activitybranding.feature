@theme @theme_boost_union_dhoch3 @theme_boost_union_dhoch3_looksettings @theme_boost_union_dhoch3_looksettings_activitybranding
Feature: Configuring the theme_boost_union_dhoch3 plugin for the "Activity branding" tab on the "Look" page
  ...
  Scenario Outline: Setting: Activity icon colors - Setting the color
    Given the following config values are set as admin:
      | config                         | value      | plugin            |
      | activityiconcolor<purposename> | <colorhex> | theme_boost_union_dhoch3 |
  ...
  Scenario Outline: Setting: Activity icon purposes - Setting the purpose
    ...
    # Testing all other locations where the activity icons are shown as well and where Boost Union Dhoch3 had to modify
  ...
  Scenario Outline: Setting: Custom icons files - Upload custom icons files
    Given the following config values are set as admin:
      | config         | value | plugin            |
      | modiconsenable | yes   | theme_boost_union_dhoch3 |
    When I log in as "admin"
    And I navigate to "Appearance > Boost Union Dhoch3 > Look" in site administration
    And I click on "Activity branding" "link" in the "#adminsettings .nav-tabs" "css_element"
    ...
    And I upload "theme/boost_union_dhoch3/tests/fixtures/<iconfile>" file to "Custom icons files" filemanager
    And I press "Save changes"
    And I navigate to "Appearance > Boost Union Dhoch3 > Look" in site administration
    And I click on "Activity branding" "link" in the "#adminsettings .nav-tabs" "css_element"
