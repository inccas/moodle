@theme @theme_boost_union_dhoch3 @theme_boost_union_dhoch3_looksettings @theme_boost_union_dhoch3_looksettings_page
Feature: Configuring the theme_boost_union_dhoch3 plugin for the "Page" tab on the "Look" page
  ...
  Scenario: Setting: Course content max width - Overwrite the course content max width setting
    Given the following config values are set as admin:
      | config                | value | plugin            |
      | coursecontentmaxwidth | 600px | theme_boost_union_dhoch3 |
  ...
  Scenario: Setting: Medium content max width - Overwrite the medium content max width setting
    Given the following config values are set as admin:
      | config                | value | plugin            |
      | mediumcontentmaxwidth | 600px | theme_boost_union_dhoch3 |
    And the theme cache is purged and the theme is reloaded
  ...
  Scenario: Setting: Course index drawer width - Overwrite the course index drawer width setting
    Given the following config values are set as admin:
      | config                 | value | plugin            |
      | courseindexdrawerwidth | 400px | theme_boost_union_dhoch3 |
    And the theme cache is purged and the theme is reloaded
    When I log in as "admin"
    And I am on "Course 1" course homepage
  ...
  Scenario: Setting: Block drawer width - Overwrite the block drawer width setting
    Given the following config values are set as admin:
      | config           | value | plugin            |
      | blockdrawerwidth | 400px | theme_boost_union_dhoch3 |
    And the theme cache is purged and the theme is reloaded
    When I log in as "admin"
    And I am on "Course 1" course homepage
    And I turn editing mode on
