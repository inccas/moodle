...
/**
 * Theme Boost Union Dhoch3 - Upgrade script.
 *
...
/**
 * Function to upgrade theme_boost_union_dhoch3
 * @param int $oldversion the version we are upgrading from
 * @return bool result
 */
function xmldb_theme_boost_union_dhoch3_upgrade($oldversion) {
    global $DB, $OUTPUT;
    ...
    if ($oldversion < 2022080916) {
        ...
        // Define table theme_boost_union_dhoch3_flavours to be created.
        $table = new xmldb_table('theme_boost_union_dhoch3_flavours');

        // Adding fields to table theme_boost_union_dhoch3_flavours.
        ...
        // Adding keys to table theme_boost_union_dhoch3_flavours.
        $table->add_key('primary', XMLDB_KEY_PRIMARY, ['id']);

        // Conditionally launch create table for theme_boost_union_dhoch3_flavours.
        if (!$dbman->table_exists($table)) {
            $dbman->create_table($table);
        }

        // Boost_union savepoint reached.
        upgrade_plugin_savepoint(true, 2022080916, 'theme', 'boost_union_dhoch3');
    }

    if ($oldversion < 2022080922) {

        // Start composing the notification to inform the admin.
        $message = html_writer::tag('p', get_string('upgradenotice_2022080922', 'theme_boost_union_dhoch3'));

        // Handle the logo and compact logo (which have got now new settings in Boost Union Dhoch3).
        foreach (['logo', 'logocompact'] as $setting) {
            ...
            if (!empty($logocore)) {
                ...
                if ($files) {
                    ...
                    $filerecord = [
                            'component' => 'theme_boost_union_dhoch3',
                            'filearea' => $setting,
                    ];

                    // Copy the logo file to Boost Union Dhoch3.
                    $newfile = $fs->create_file_from_storedfile($filerecord, $file);

                    // Set the theme config to the file name.
                    set_config($setting, '/'.$newfile->get_filename(), 'theme_boost_union_dhoch3');
            ...
            if ($logocopied == true) {
                ...
                $message .= html_writer::tag('p', get_string('upgradenotice_2022080922_copied', 'theme_boost_union_dhoch3',
                        get_string('upgradenotice_2022080922_'.$setting, 'theme_boost_union_dhoch3')));

                // Otherwise, if no logo was copied.
            } else {
                // Add the corresponding note to the notification.
                $message .= html_writer::tag('p', get_string('upgradenotice_2022080922_notcopied', 'theme_boost_union_dhoch3',
                        get_string('upgradenotice_2022080922_'.$setting, 'theme_boost_union_dhoch3')));
            }
        }
        ...
        // Boost_union savepoint reached.
        upgrade_plugin_savepoint(true, 2022080922, 'theme', 'boost_union_dhoch3');
    }

    if ($oldversion < 2023010517) {

        // Define table theme_boost_union_dhoch3_menus to be created.
        $table = new xmldb_table('theme_boost_union_dhoch3_menus');

        // Adding fields to table theme_boost_union_dhoch3_menus.
        ...
        // Adding keys to table theme_boost_union_dhoch3_menus.
        $table->add_key('primary', XMLDB_KEY_PRIMARY, ['id']);

        // Conditionally launch create table for theme_boost_union_dhoch3_menus.
        if (!$dbman->table_exists($table)) {
            $dbman->create_table($table);
        }

        // Define table theme_boost_union_dhoch3_menuitems to be created.
        $table = new xmldb_table('theme_boost_union_dhoch3_menuitems');

        // Adding fields to table theme_boost_union_dhoch3_menuitems.
        $table->add_field('id', XMLDB_TYPE_INTEGER, '18', XMLDB_UNSIGNED, XMLDB_NOTNULL, XMLDB_SEQUENCE);
        $table->add_field('title', XMLDB_TYPE_CHAR, '255', null, null);
        ...
        // Adding keys to table theme_boost_union_dhoch3_menuitems.
        $table->add_key('primary', XMLDB_KEY_PRIMARY, ['id']);

        // Conditionally launch create table for theme_boost_union_dhoch3_menuitems.
        if (!$dbman->table_exists($table)) {
            $dbman->create_table($table);
        }

        // Boost_union savepoint reached.
        upgrade_plugin_savepoint(true, 2023010517, 'theme', 'boost_union_dhoch3');
    }

    if ($oldversion < 2023010519) {

        // Define table theme_boost_union_dhoch3_menus to be altered.
        $table = new xmldb_table('theme_boost_union_dhoch3_menus');
        ...
        // Boost_union savepoint reached.
        upgrade_plugin_savepoint(true, 2023010519, 'theme', 'boost_union_dhoch3');
    }

    if ($oldversion < 2023010521) {

        // Remove the THEME_BOOST_UNION_DHOCH3_SETTING_COURSEBREADCRUMBS_DONTCHANGE option from the categorybreadcrumbs setting
        // and replace it with THEME_BOOST_UNION_DHOCH3_SETTING_SELECT_NO if necessary.
        $oldconfig = get_config('theme_boost_union_dhoch3', 'categorybreadcrumbs');
        if ($oldconfig == 'dontchange') {
            set_config('categorybreadcrumbs', THEME_BOOST_UNION_DHOCH3_SETTING_SELECT_NO, 'theme_boost_union_dhoch3');
        }

        // Boost_union savepoint reached.
        upgrade_plugin_savepoint(true, 2023010521, 'theme', 'boost_union_dhoch3');
    }

    if ($oldversion < 2023090100) {
        // Remove the files from the FontAwesome filearea of Boost Union Dhoch3 as this setting was removed.
        $systemcontext = context_system::instance();
        $fs = get_file_storage();
        $fs->delete_area_files($systemcontext->id, 'theme_boost_union_dhoch3', 'fontawesome');

        // Boost_union savepoint reached.
        upgrade_plugin_savepoint(true, 2023090100, 'theme', 'boost_union_dhoch3');
    }
    ...
    if ($oldversion < 2023102008) {
        ...
        // Define field listsort to be added to theme_boost_union_dhoch3_menuitems.
        ...
        // Boost_union savepoint reached.
        upgrade_plugin_savepoint(true, 2023102008, 'theme', 'boost_union_dhoch3');
    }

    if ($oldversion < 2023102009) {

        // Changing precision of field description_format on table theme_boost_union_dhoch3_flavours to (2).
        $table = new xmldb_table('theme_boost_union_dhoch3_flavours');
        ...
        // Boost_union savepoint reached.
        upgrade_plugin_savepoint(true, 2023102009, 'theme', 'boost_union_dhoch3');
    }

    if ($oldversion < 2023102021) {

        // Remove the preset setting from Boost Union Dhoch3.
        unset_config('preset', 'theme_boost_union_dhoch3');

        // Remove the presetfiles setting from Boost Union Dhoch3.
        unset_config('presetfiles', 'theme_boost_union_dhoch3');

        // Remove the files from the preset filearea of Boost Union Dhoch3 as this setting was removed.
        $systemcontext = context_system::instance();
        $fs = get_file_storage();
        $fs->delete_area_files($systemcontext->id, 'theme_boost_union_dhoch3', 'preset');

        // Boost_union savepoint reached.
        upgrade_plugin_savepoint(true, 2023102021, 'theme', 'boost_union_dhoch3');
    }

    if ($oldversion < 2023102027) {
        // Define table theme_boost_union_dhoch3_menuitems to be altered.
        $table = new xmldb_table('theme_boost_union_dhoch3_menuitems');
        ...
        // Boost_union savepoint reached.
        upgrade_plugin_savepoint(true, 2023102027, 'theme', 'boost_union_dhoch3');
    }
