...
/**
 * Theme Boost Union Dhoch3 - Core renderer
 *
...
class core_renderer extends \theme_boost\output\core_renderer {
    ...
    public function favicon() {
            ...
            if ($flavour != null) {
                ...
                if ($flavour->look_favicon != null) {
                    ...
                    $flavourfaviconurl = moodle_url::make_pluginfile_url(
                            context_system::instance()->id, 'theme_boost_union_dhoch3', 'flavours_look_favicon', $flavour->id,
        ...
        if (!during_initial_install()) {
            $logo = get_config('theme_boost_union_dhoch3', 'favicon');
        }
        if (empty($logo)) {
        ...
        return moodle_url::make_pluginfile_url(context_system::instance()->id, 'theme_boost_union_dhoch3', 'favicon', '64x64/',
    ...
    public function get_logo_url($maxwidth = null, $maxheight = 200) {
            ...
            if ($flavour != null) {
                ...
                if ($flavour->look_logo != null) {
                    ...
                    $flavourlogourl = moodle_url::make_pluginfile_url(
                            context_system::instance()->id, 'theme_boost_union_dhoch3', 'flavours_look_logo', $flavour->id,
        ...
        $logo = get_config('theme_boost_union_dhoch3', 'logo');
        if (empty($logo)) {
            return false;
        }
        ...
        if (in_array($logoextension, ['svg', 'svgz'])) {
            // The theme_boost_union_dhoch3_pluginfile() function will look for a filepath and will try to extract the size from that.
            // Thus, we cannot drop the filepath from the URL completely.
            // But we can add a path without an 'x' in it which will then be interpreted by theme_boost_union_dhoch3_pluginfile()
        ...
        // Use $CFG->themerev to prevent browser caching when the file changes.
        return moodle_url::make_pluginfile_url(context_system::instance()->id, 'theme_boost_union_dhoch3', 'logo', $filepath,
                theme_get_revision(), $logo);
    }
    ...
    public function get_compact_logo_url($maxwidth = 300, $maxheight = 300) {
        ...
        if ($hasflavourlogo != null) {
            ...
            if ($flavour != null) {
                ...
                if ($flavour->look_logocompact != null) {
                    ...
                    $flavourlogourl = moodle_url::make_pluginfile_url(
                            context_system::instance()->id, 'theme_boost_union_dhoch3', 'flavours_look_logocompact', $flavour->id,
        ...
        $logo = get_config('theme_boost_union_dhoch3', 'logocompact');
        if (empty($logo)) {
            return false;
        }
        ...
        // Use $CFG->themerev to prevent browser caching when the file changes.
        return moodle_url::make_pluginfile_url(context_system::instance()->id, 'theme_boost_union_dhoch3', 'logocompact', $filepath,
                theme_get_revision(), $logo);
    }
    ...
    public function body_attributes($additionalclasses = []) {
        ...
        if ($this->page->pagelayout != 'login') {
            if (!empty(get_config('theme_boost_union_dhoch3', 'backgroundimage'))) {
        ...
        if ($this->page->pagelayout == 'login') {
            ...
            $loginimageclass = theme_boost_union_dhoch3_get_random_loginbackgroundimage_class();
        ...
        // Boost Union Dhoch3 itself does not need this class for applying the flavour to the page (yet).
        ...
        $footerbutton = get_config('theme_boost_union_dhoch3', 'enablefooterbutton');
        switch ($footerbutton) {
            case THEME_BOOST_UNION_DHOCH3_SETTING_ENABLEFOOTER_NONE:
                $additionalclasses[] = 'theme_boost-union-dhoch3-footerbuttonnone';
                break;
            case THEME_BOOST_UNION_DHOCH3_SETTING_ENABLEFOOTER_ALL:
                $additionalclasses[] = 'theme_boost-union-dhoch3-footerbuttonall';
                break;
            case THEME_BOOST_UNION_DHOCH3_SETTING_ENABLEFOOTER_MOBILE:
                $additionalclasses[] = 'theme_boost-union-dhoch3-footerbuttonmobile';
                break;
            case THEME_BOOST_UNION_DHOCH3_SETTING_ENABLEFOOTER_DESKTOP:
            default:
                $additionalclasses[] = 'theme_boost-union-dhoch3-footerbuttondesktop';
                break;
        }
    ...
    public function full_header() {
        ...
        if ($this->page->pagelayout == 'course' && (get_config('theme_boost_union_dhoch3', 'courseheaderimageenabled')
            ...
            $header->courseheaderimageurl = theme_boost_union_dhoch3_get_course_header_image_url();
            // Additionally, get the course header image height.
            $header->courseheaderimageheight = get_config('theme_boost_union_dhoch3', 'courseheaderimageheight');
            // Additionally, get the course header image position.
            $header->courseheaderimageposition = get_config('theme_boost_union_dhoch3', 'courseheaderimageposition');
            // Additionally, get the template context attributes for the course header image layout.
            $courseheaderimagelayout = get_config('theme_boost_union_dhoch3', 'courseheaderimagelayout');
            switch($courseheaderimagelayout) {
                case THEME_BOOST_UNION_DHOCH3_SETTING_COURSEIMAGELAYOUT_HEADINGABOVE:
                    $header->courseheaderimagelayoutheadingabove = true;
                    $header->courseheaderimagelayoutstackedclass = '';
                    break;
                case THEME_BOOST_UNION_DHOCH3_SETTING_COURSEIMAGELAYOUT_STACKEDDARK:
                    $header->courseheaderimagelayoutheadingabove = false;
                    $header->courseheaderimagelayoutstackedclass = 'dark';
                    break;
                case THEME_BOOST_UNION_DHOCH3_SETTING_COURSEIMAGELAYOUT_STACKEDLIGHT:
                    $header->courseheaderimagelayoutheadingabove = false;
    ...
    public function navbar(): string {
        $newnav = new \theme_boost_union_dhoch3\boostnavbar($this->page);
    ...
    public function block(\block_contents $bc, $region) {
        ...
        $regions = theme_boost_union_dhoch3_get_additional_regions();
        $regioncapname = array_search($region, $regions);
        if (!empty($regioncapname) && $context->hascontrols) {
            $context->hascontrols = has_capability('theme/boost_union_dhoch3:editregion'.$regioncapname, $this->page->context);
        }
    ...
    public function render_login(\core_auth\output\login $form) {
        ...
        $loginlocalloginsetting = get_config('theme_boost_union_dhoch3', 'loginlocalloginenable');
        $showlocallogin = ($loginlocalloginsetting != false) ? $loginlocalloginsetting : THEME_BOOST_UNION_DHOCH3_SETTING_SELECT_YES;
        if ($showlocallogin == THEME_BOOST_UNION_DHOCH3_SETTING_SELECT_YES) {
        ...
        $loginlocalshowintrosetting = get_config('theme_boost_union_dhoch3', 'loginlocalshowintro');
        $showlocalloginintro = ($loginlocalshowintrosetting != false) ?
            $loginlocalshowintrosetting : THEME_BOOST_UNION_DHOCH3_SETTING_SELECT_NO;
        if ($showlocalloginintro == THEME_BOOST_UNION_DHOCH3_SETTING_SELECT_YES) {
        ...
        $loginidpshowintrosetting = get_config('theme_boost_union_dhoch3', 'loginidpshowintro');
        $showidploginintro = ($loginidpshowintrosetting != false) ?
                $loginidpshowintrosetting : THEME_BOOST_UNION_DHOCH3_SETTING_SELECT_YES;
        if ($showidploginintro == THEME_BOOST_UNION_DHOCH3_SETTING_SELECT_YES) {
    ...
    public function standard_footer_html() {
        ...
        foreach ($pluginswithfunction as $plugintype => $plugins) {
            foreach ($plugins as $pluginname => $function) {
                // If the given plugin's output is supressed by Boost Union Dhoch3's settings.
                $suppresssetting = get_config('theme_boost_union_dhoch3', 'footersuppressstandardfooter_'.$plugintype.'_'.$pluginname);
                if (isset($suppresssetting) && $suppresssetting == THEME_BOOST_UNION_DHOCH3_SETTING_SELECT_YES) {
                    // Skip the plugin.
                    continue;
        ...
        // If the 'Give feedback about this software' link is not suppressed by Boost Union Dhoch3's settings.
        $suppressfeedbacksetting = get_config('theme_boost_union_dhoch3', 'footersuppressfeedback');
        if (!isset($suppressfeedbacksetting) || $suppressfeedbacksetting == THEME_BOOST_UNION_DHOCH3_SETTING_SELECT_NO) {
        ...
        // If the theme switcher links are not suppressed by Boost Union Dhoch3's settings.
        $suppressthemeswitchsetting = get_config('theme_boost_union_dhoch3', 'footersuppressthemeswitch');
        if (!isset($suppressthemeswitchsetting) || $suppressthemeswitchsetting == THEME_BOOST_UNION_DHOCH3_SETTING_SELECT_NO) {
