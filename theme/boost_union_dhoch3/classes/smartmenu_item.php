...
class smartmenu_item {
    ...
    public function __construct($item, $menu=null) {
        ...
        $this->cache = cache::make('theme_boost_union_dhoch3', 'smartmenu_items');

        // Menus cache instance.
        // Purge the menu related to the item, when the item is updated, created and sorted.
        $this->menucache = cache::make('theme_boost_union_dhoch3', 'smartmenus');
    }
    ...
    public function get_item($itemid=null) {
        ...
        if ($record = $DB->get_record('theme_boost_union_dhoch3_menuitems', ['id' => $itemid ?: $this->id ])) {
    ...
    public function delete_menuitem() {
        ...
        if ($DB->delete_records('theme_boost_union_dhoch3_menuitems', ['id' => $this->id])) {
            // Reorder the items.
            $this->reorder_items();
            // Delete the cache.
            $this->delete_cache();
    ...
    public function move_upward() {
        ...
        if ($currentposition > 1) {
            ...
            $sql = 'SELECT * FROM {theme_boost_union_dhoch3_menuitems} WHERE sortorder < :pos AND menu = :menu ORDER BY sortorder ASC';
            ...
            $DB->set_field('theme_boost_union_dhoch3_menuitems', 'sortorder', $previtem->sortorder, [
                'id' => $this->id,
                'menu' => $this->item->menu,
            ]);
            // Set the prevmenu position to down.
            $DB->set_field('theme_boost_union_dhoch3_menuitems', 'sortorder', $currentposition, [
                'id' => $previtem->id,
                'menu' => $this->item->menu,
            ]);
    ...
    public function move_downward() {
        ...
        $DB->set_field('theme_boost_union_dhoch3_menuitems', 'sortorder', $nextitem->sortorder, [
            'id' => $this->id,
            'menu' => $this->item->menu,
        ]);
    ...
    public function reorder_items() {
        ...
        $sql = 'SELECT * FROM {theme_boost_union_dhoch3_menuitems} WHERE menu=:menu ORDER BY sortorder ASC';
        ...
        if (end($records)->sortorder != count($records)) {
            ...
            foreach ($records as $itemid => $item) {
                $DB->set_field('theme_boost_union_dhoch3_menuitems', 'sortorder', $i, ['id' => $item->id]);
                $i++;
            }
        }
    }
    ...
    public static function get_lastitem($menuid) {
        ...
        $sql = 'SELECT * FROM {theme_boost_union_dhoch3_menuitems} WHERE menu=:menu ORDER BY sortorder DESC';
    ...
    public function duplicate() {
        ...
        if (self::manage_instance($record)) {
            \core\notification::success(get_string('smartmenusmenuitemduplicatesuccess', 'theme_boost_union_dhoch3'));
        }
    }
    ...
    public function update_field($key, $value) {
        ...
        return $DB->set_field('theme_boost_union_dhoch3_menuitems', $key, $value, ['id' => $this->id]);
    }
    ...
    public function get_itemimage($itemid) {
        ...
        $files = $fs->get_area_files($contextid, 'theme_boost_union_dhoch3', 'smartmenus_itemimage', $itemid, '', false);
        if (!empty($files)) {
            // Get the first file.
            $file = reset($files);
    ...
    public static function get_itemscount($menuid) {
        global $DB;
        return $DB->count_records('theme_boost_union_dhoch3_menuitems', ['menu' => $menuid]);
    }
    ...
    public static function get_types(int $type=null) {
        $types = [
                ...
                self::TYPEDYNAMIC => get_string('smartmenusmenuitemtypedynamiccourses', 'theme_boost_union_dhoch3'),
        ];
    ...
    public static function manage_instance($formdata) {
        ...
        // Cache for menus.
        $menucache = cache::make('theme_boost_union_dhoch3', 'smartmenus');
        // Cache for menu items.
        $cache = cache::make('theme_boost_union_dhoch3', 'smartmenu_items');

        if (isset($formdata->id) && $oldrecord = $DB->get_record('theme_boost_union_dhoch3_menuitems', ['id' => $formdata->id])) {
            $itemid = $formdata->id;

            $DB->update_record('theme_boost_union_dhoch3_menuitems', $record);
            ...
            if ($oldrecord->sortorder != $record->sortorder) {
                ...
                $decrease = "UPDATE {theme_boost_union_dhoch3_menuitems} $operation AND id != :item AND menu=:menuid";
            ...
            \core\notification::success(get_string('smartmenusmenuitemeditsuccess', 'theme_boost_union_dhoch3'));
        } else {
            $record->sortorder = $record->sortorder ?: 1;
            $itemid = $DB->insert_record('theme_boost_union_dhoch3_menuitems', $record);
            // Setup the order for item.
            $sql = "UPDATE {theme_boost_union_dhoch3_menuitems}
            ...
            \core\notification::success(get_string('smartmenusmenuitemcreatesuccess', 'theme_boost_union_dhoch3'));
