...
/**
 * Theme Boost Union Dhoch3 - Flavours library
 *
 ...
 */
function theme_boost_union_dhoch3_get_flavour_which_applies() {
    ...
    if (get_config('theme_boost_union_dhoch3', 'version') < 2022080916) {
        return null;
    }
    ...
    if ($flavourchecked == true) {
        ...
        $previewurl = new moodle_url('/theme/boost_union_dhoch3/flavours/preview.php');
        if ($previewurl->compare($PAGE->url, URL_MATCH_BASE) == true) {
            ...
            $appliedflavour = $DB->get_record('theme_boost_union_dhoch3_flavours', ['id' => $previewflavourid]);
        ...
        if (get_user_preferences('theme_boost_union_dhoch3_flavours_purgesessioncache', false) == true) {
            // Purge the flavours cache for this user.
            cache_helper::purge_by_definition('theme_boost_union_dhoch3', 'flavours');
        }

        // Create cache for flavours.
        $cache = cache::make('theme_boost_union_dhoch3', 'flavours');
        ...
        $flavours = $DB->get_records('theme_boost_union_dhoch3_flavours', [], 'sort ASC');
            ...
            if ($f->applytocohorts == true) {
                ...
                if (!empty($usercohorts)) {
                    ...
                    if (!empty($cohortids)) {
                        ...
                        if (theme_boost_union_dhoch3_flavours_cohorts_is_member($USER->id, $cohortids) == true) {
                            // Remember the flavour object as we have found a match.
                            $appliedflavour = $f;
 ...
 */
function theme_boost_union_dhoch3_flavours_get_filename($filearea, $itemid) {
    // Get system context.
    ...
    $files = $fs->get_area_files($context->id, 'theme_boost_union_dhoch3', 'flavours_'.$filearea, $itemid,
 ...
 * @return bool
 */
function theme_boost_union_dhoch3_flavours_cohorts_is_member($userid, $cohorts) {
    global $DB;
 ...
 * @return bool
 */
function theme_boost_union_dhoch3_flavour_exists_for_cohort($cohortid) {
    global $DB;

    // Get the flavours which are configured to apply to (any) cohort.
    $flavoursforcohorts = $DB->get_records('theme_boost_union_dhoch3_flavours', ['applytocohorts' => 1], '', 'applytocohorts_ids');
