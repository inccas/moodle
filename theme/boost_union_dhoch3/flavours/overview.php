...
/**
 * Theme Boost Union Dhoch3 - Flavours overview page
 *
...
// Access checks.
admin_externalpage_setup('theme_boost_union_dhoch3_flavours');
...
$PAGE->set_url(new moodle_url('/theme/boost_union_dhoch3/flavours/overview.php'));
...
if ($action !== null && confirm_sesskey()) {
    ...
    switch ($action) {
        case 'up':
            ...
            $currentflavour = $DB->get_record('theme_boost_union_dhoch3_flavours', ['id' => $flavourid]);
            $prevflavour = $DB->get_record('theme_boost_union_dhoch3_flavours', ['sort' => $currentflavour->sort - 1]);
            if ($prevflavour) {
                $DB->set_field('theme_boost_union_dhoch3_flavours', 'sort', $prevflavour->sort,
                        ['id' => $currentflavour->id]);
                $DB->set_field('theme_boost_union_dhoch3_flavours', 'sort', $currentflavour->sort,
                        ['id' => $prevflavour->id]);
                ...
                cache_helper::purge_by_event('theme_boost_union_dhoch3_flavours_resorted');
            }
            break;
        case 'down':
            // Move the flavour downwards.
            $currentflavour = $DB->get_record('theme_boost_union_dhoch3_flavours', ['id' => $flavourid]);
            $nextflavour = $DB->get_record('theme_boost_union_dhoch3_flavours', ['sort' => $currentflavour->sort + 1]);
            if ($nextflavour) {
                $DB->set_field('theme_boost_union_dhoch3_flavours', 'sort', $nextflavour->sort,
                        ['id' => $currentflavour->id]);
                $DB->set_field('theme_boost_union_dhoch3_flavours', 'sort', $currentflavour->sort,
                        ['id' => $nextflavour->id]);
...
$PAGE->set_title(theme_boost_union_dhoch3_get_externaladminpage_title(get_string('flavoursflavours', 'theme_boost_union_dhoch3')));
$PAGE->set_heading(theme_boost_union_dhoch3_get_externaladminpage_heading());

// Build flavours table.
$table = new \theme_boost_union_dhoch3\table\flavours_overview();
$table->define_baseurl($PAGE->url);

// Start page output.
echo $OUTPUT->header();
echo $OUTPUT->heading(get_string('configtitleflavours', 'theme_boost_union_dhoch3'));

// Show flavours description.
echo get_string('flavoursoverview_desc', 'theme_boost_union_dhoch3');

// Prepare 'Create flavours' button.
$createbutton = $OUTPUT->box_start();
$createbutton .= $OUTPUT->single_button(
        new \moodle_url('/theme/boost_union_dhoch3/flavours/edit.php', ['action' => 'create', 'sesskey' => sesskey()]),
        get_string('flavourscreateflavour', 'theme_boost_union_dhoch3'), 'get');
