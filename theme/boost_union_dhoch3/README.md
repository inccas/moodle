moodle-theme_boost_union_dhoch3
========================

[![Moodle Plugin CI](https://github.com/moodle-an-hochschulen/moodle-theme_boost_union_dhoch3/workflows/Moodle%20Plugin%20CI/badge.svg?branch=MOODLE_403_STABLE)](https://github.com/moodle-an-hochschulen/moodle-theme_boost_union_dhoch3/actions?query=workflow%3A%22Moodle+Plugin+CI%22+branch%3AMOODLE_403_STABLE)

Theme Boost Union Dhoch3 is an enhanced child theme of Boost which is intended, on the one hand, to make Boost simply more configurable and, on the other hand, to provide helpful additional features for the daily Moodle operation of admins, teachers and students.
...
One highlight is the main design principle of Boost Union Dhoch3: As soon as it is activated on a Moodle site, it does not change anything yet and simply behaves as Boost from Moodle core does. The admin can enable and configure only the theme features he needs and does not need to care about side effects from other, disabled theme features.

As a side note, it is quite easy to create a grandchild theme of Boost Union Dhoch3. See the 'Grandchild theme support' section below for details.
...
Site administration -> Appearance -> Boost Union Dhoch3.
...
Theme presets can be used to dramatically alter the appearance of the theme. Boost Union Dhoch3 does not re-implement the theme preset setting. If you want to use theme presets, please set them directly in Boost. Boost Union Dhoch3 will inherit and use the configured preset.
...
This setting is already available in the Moodle core theme Boost. For more information how to use it, please have a look at the official Moodle documentation: http://docs.moodle.org/en/Boost_theme
Please note: This will not interfere with the setting "theme_boost_union_dhoch3 | loginbackgroundimage" which means that the pictures uploaded here will be shown on all pages except the login page.
...
However, in Boost Union Dhoch3 you can not only add one but up to 25 files as a background image for the login page. One of these images will be picked randomly and shown when the user visits the login page.
...
Please note: As soon as you hide the local login form, you risk that admins cannot log in anymore with a local account if there is a problem with the external identity provider. To allow local logins anyway in such cases, a side entrance local login page is provided on /theme/boost_union_dhoch3/locallogin.php. On this side entrance local login page, all of Moodle's login security measures apply as well.
...
With this setting a back to top button will appear in the bottom right corner of the page as soon as the user scrolls down the page. A button like this existed already on Boost in Moodle Core until Moodle 3.11, but was removed in 4.0. With Boost Union Dhoch3, you can bring it back.
...
Boost Union Dhoch3 provides a large number of additional block regions which can be used to add and show blocks over the whole Moodle page:
...
With this setting, you control the scope where Boost Union Dhoch3 should mark mailto links. By default, Boost Union Dhoch3 marks mailto links on the whole Moodle page. However, you can also limit the scope to avoid edge-cases.
...
Boost Union Dhoch3's flavours offer a possibility to override particular Moodle look & feel settings in particular contexts. On this page, you can create and manage flavours.
...
### theme/boost_union_dhoch3:viewregion*
...
### theme/boost_union_dhoch3:editregion*
...
* Themerev as SCSS variable:
  During a custom SCSS design project, you might come into the situation that you have to link to an uploaded image or other asset which is served by Moodle's pluginfile.php script. Unfortunately, these URLs contain a theme revision parameter. To be able to use these URLs properly in custom SCSS and to avoid breaking Moodle's caching features, Boost Union Dhoch3 provides the $themerev SCSS variable to be used in your custom SCSS.


Exceptions to our main design principle
---------------------------------------

As you have read in the introduction, the main design principle of Boost Union Dhoch3 is not to change anything in the GUI until a particular feature is enabled in the theme settings. However, due to the way how Moodle core and Boost in Moodle core is built, this main design principle sometimes could not be fully satisfied:
...
* block_dash / local_dash:
  * Style improvements for Dash dashboards in combination with Boost Union Dhoch3 block regions
* local_learningtools:
  * Style improvements for the learning tools button in combination with the Boost Union Dhoch3 bottom navigation
...
It is quite easy to create a grandchild theme of Boost Union Dhoch3. That way, you can benefit from all the / only the Boost Union Dhoch3 features you need, but you can also add additional local features or settings to your local grandchild theme at the same time.

If you plan to build a grandchild theme of Boost Union Dhoch3, we have prepared a 'Boost Union Dhoch3 Child' boilerplate for you which can help you to do the first steps.

Boost Union Dhoch3 Child can be found on Github:
https://github.com/moodle-an-hochschulen/moodle-theme_boost_union_dhoch3_child

While Boost Union Dhoch3 Child will surely help you to realize all your local Boost Union Dhoch3 dreams, please do yourself and the whole community a favour and verify that your planned features are indeed not interesting as a pull request or feature request for the whole Boost Union Dhoch3 community and could be contributed to Boost Union Dhoch3 directly instead.
...
http://moodle.org/plugins/view/theme_boost_union_dhoch3

The latest development version can be found on Github:
https://github.com/moodle-an-hochschulen/moodle-theme_boost_union_dhoch3
...
https://github.com/moodle-an-hochschulen/moodle-theme_boost_union_dhoch3/issues

Support thread:
https://moodle.org/mod/forum/discuss.php?d=452129

We kindly invite you to use this support thread in case of any questions you might have. We are a team of many (sometimes power) users of Boost Union Dhoch3 and will try to answer or collectively according to our measures. If any other users know answers or are quicker, don't hesitate to answer. We will do our best to solve your problems, but please note that due to limited resources we can't always provide per-case support.
...
https://github.com/moodle-an-hochschulen/moodle-theme_boost_union_dhoch3/pulls
