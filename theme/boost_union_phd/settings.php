...
/**
 * Theme Boost Union PhD - Settings file
 *
...
defined('THEME_BOOST_UNION_PHD_SETTING_INHERITANCE_INHERIT') || define('THEME_BOOST_UNION_PHD_SETTING_INHERITANCE_INHERIT', 0);
defined('THEME_BOOST_UNION_PHD_SETTING_INHERITANCE_DUPLICATE') || define('THEME_BOOST_UNION_PHD_SETTING_INHERITANCE_DUPLICATE', 1);
...
if ($hassiteconfig || has_capability('theme/boost_union:configure', context_system::instance())) {
    ...
    // To avoid that there appears a broken "Boost Union PhD" settings page, we redirect the user to a settings
    // overview page if he opens this page.
    $mainsettingspageurl = new \core\url('/admin/settings.php', ['section' => 'themesettingboost_union_phd']);
    if ($ADMIN->fulltree && $PAGE->has_set_url() && $PAGE->url->compare($mainsettingspageurl)) {
        redirect(new \core\url('/admin/settings.php', ['section' => 'theme_boost_union_phd']));
    }

    // Create empty settings page structure to make the site administration work on non-admin pages.
    if (!$ADMIN->fulltree) {
        // Create Boost Union PhD settings page
        // (and allow users with the theme/boost_union:configure capability to access it).
        $tab = new admin_settingpage('theme_boost_union_phd',
                get_string('configtitle', 'theme_boost_union_phd', null, true),
    ...
    // phpcs:disable moodle.ControlStructures.ControlSignature.Found
        ...
        // Create Boost Union PhD settings page with tabs
        // (and allow users with the theme/boost_union:configure capability to access it).
        $page = new theme_boost_admin_settingspage_tabs('theme_boost_union_phd',
                get_string('configtitle', 'theme_boost_union_phd', null, true),
                'theme/boost_union:configure');


        // Create general settings tab.
        $tab = new admin_settingpage('theme_boost_union_phd_general',
                get_string('generalsettings', 'theme_boost', null, true));

        // Create inheritance heading.
        $name = 'theme_boost_union_phd/inheritanceheading';
        $title = get_string('inheritanceheading', 'theme_boost_union_phd', null, true);
        $setting = new admin_setting_heading($name, $title, null);
        $tab->add($setting);

        // Prepare inheritance options.
        $inheritanceoptions = [
                THEME_BOOST_UNION_PHD_SETTING_INHERITANCE_INHERIT =>
                        get_string('inheritanceinherit', 'theme_boost_union_phd'),
                THEME_BOOST_UNION_PHD_SETTING_INHERITANCE_DUPLICATE =>
                        get_string('inheritanceduplicate', 'theme_boost_union_phd'),
        ];

        // Setting: Pre SCSS inheritance setting.
        $name = 'theme_boost_union_phd/prescssinheritance';
        $title = get_string('prescssinheritancesetting', 'theme_boost_union_phd', null, true);
        $description = get_string('prescssinheritancesetting_desc', 'theme_boost_union_phd', null, true).'<br />'.
                get_string('inheritanceoptionsexplanation', 'theme_boost_union_phd', null, true);
        $setting = new admin_setting_configselect($name, $title, $description,
                THEME_BOOST_UNION_PHD_SETTING_INHERITANCE_INHERIT, $inheritanceoptions);
        $setting->set_updatedcallback('theme_reset_all_caches');
        $tab->add($setting);

        // Setting: Extra SCSS inheritance setting.
        $name = 'theme_boost_union_phd/extrascssinheritance';
        $title = get_string('extrascssinheritancesetting', 'theme_boost_union_phd', null, true);
        $description = get_string('extrascssinheritancesetting_desc', 'theme_boost_union_phd', null, true).'<br />'.
                get_string('inheritanceoptionsexplanation', 'theme_boost_union_phd', null, true);
        $setting = new admin_setting_configselect($name, $title, $description,
                THEME_BOOST_UNION_PHD_SETTING_INHERITANCE_INHERIT, $inheritanceoptions);
        $setting->set_updatedcallback('theme_reset_all_caches');
