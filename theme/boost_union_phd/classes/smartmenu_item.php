...
namespace theme_boost_union_phd;
...
class smartmenu_item {
    ...
    public function __construct($item, $menu = null) {
        ...
        if (!is_array($item) && !is_object($item)) {
            throw new \moodle_exception('error:smartmenusmenuitemnotfound', 'theme_boost_union_phd');
        }
        ...
        $this->cache = cache::make('theme_boost_union_phd', 'smartmenu_items');

        // Menus cache instance.
        // Purge the menu related to the item, when the item is updated, created and sorted.
        $this->menucache = cache::make('theme_boost_union_phd', 'smartmenus');
    }
    ...
    public function get_item($itemid = null) {
        ...
        if ($record = $DB->get_record('theme_boost_union_phd_menuitems', ['id' => $itemid ?: $this->id ])) {
    ...
    public function delete_menuitem() {
        ...
        if ($DB->delete_records('theme_boost_union_phd_menuitems', ['id' => $this->id])) {
            // Reorder the items.
            $this->reorder_items();
            // Delete the cache.
            $this->delete_cache();
    ...
    public function move_upward() {
        ...
        if ($currentposition > 1) {
            ...
            $sql = 'SELECT * FROM {theme_boost_union_phd_menuitems} WHERE sortorder < :pos AND menu = :menu ORDER BY sortorder ASC';
            ...
            $DB->set_field('theme_boost_union_phd_menuitems', 'sortorder', $previtem->sortorder, [
                'id' => $this->id,
                'menu' => $this->item->menu,
            ]);
            // Set the prevmenu position to down.
            $DB->set_field('theme_boost_union_phd_menuitems', 'sortorder', $currentposition, [
                'id' => $previtem->id,
                'menu' => $this->item->menu,
            ]);
    ...
    public function move_downward() {
        ...
        $DB->set_field('theme_boost_union_phd_menuitems', 'sortorder', $nextitem->sortorder, [
            'id' => $this->id,
            'menu' => $this->item->menu,
        ]);
    ...
    public function reorder_items() {
        ...
        $sql = 'SELECT * FROM {theme_boost_union_phd_menuitems} WHERE menu=:menu ORDER BY sortorder ASC';
        ...
        if (end($records)->sortorder != count($records)) {
            ...
            foreach ($records as $itemid => $item) {
                $DB->set_field('theme_boost_union_phd_menuitems', 'sortorder', $i, ['id' => $item->id]);
                $i++;
            }
        }
    }
    ...
    public static function get_lastitem($menuid) {
        ...
        $sql = 'SELECT * FROM {theme_boost_union_phd_menuitems} WHERE menu=:menu ORDER BY sortorder DESC';
    ...
    public function duplicate() {
        ...
        if (self::manage_instance($record)) {
            \core\notification::success(get_string('smartmenusmenuitemduplicatesuccess', 'theme_boost_union_phd'));
        }
    }
    ...
    public function update_field($key, $value) {
        ...
        return $DB->set_field('theme_boost_union_phd_menuitems', $key, $value, ['id' => $this->id]);
    }
    ...
    public function get_itemimage($itemid) {
        ...
        $files = $fs->get_area_files($contextid, 'theme_boost_union_phd', 'smartmenus_itemimage', $itemid, '', false);
        if (!empty($files)) {
            // Get the first file.
            $file = reset($files);
    ...
    public static function get_itemscount($menuid) {
        global $DB;
        return $DB->count_records('theme_boost_union_phd_menuitems', ['menu' => $menuid]);
    }
    ...
    public static function get_types(?int $type = null) {
        $types = [
                self::TYPESTATIC => get_string('smartmenusmenuitemtypestatic', 'theme_boost_union_phd'),
                self::TYPEHEADING => get_string('smartmenusmenuitemtypeheading', 'theme_boost_union_phd'),
                self::TYPEDYNAMIC => get_string('smartmenusmenuitemtypedynamiccourses', 'theme_boost_union_phd'),
        ];
    ...
    public static function get_display_options() {
        return [
            self::DISPLAY_SHOWTITLEICON => get_string('smartmenusmenuitemdisplayoptionsshowtitleicon', 'theme_boost_union_phd'),
            self::DISPLAY_HIDETITLE => get_string('smartmenusmenuitemdisplayoptionshidetitle', 'theme_boost_union_phd'),
            self::DISPLAY_HIDETITLEMOBILE => get_string('smartmenusmenuitemdisplayoptionshidetitlemobile', 'theme_boost_union_phd'),
        ];
    }
    ...
    public static function get_target_options(): array {
        return [
            self::TARGET_SAME => get_string('smartmenusmenuitemlinktargetsamewindow', 'theme_boost_union_phd'),
            self::TARGET_NEW => get_string('smartmenusmenuitemlinktargetnewtab', 'theme_boost_union_phd'),
        ];
    }
    ...
    public static function get_completionstatus_options(): array {
        return [
            self::COMPLETION_ENROLLED =>
                get_string('smartmenusdynamiccoursescompletionstatusenrolled', 'theme_boost_union_phd'),
            self::COMPLETION_INPROGRESS =>
                get_string('smartmenusdynamiccoursescompletionstatusinprogress', 'theme_boost_union_phd'),
            self::COMPLETION_COMPLETED =>
                get_string('smartmenusdynamiccoursescompletionstatuscompleted', 'theme_boost_union_phd'),
        ];
    }
    ...
    public static function get_daterange_options(): array {
        return [
            self::RANGE_PAST =>
                get_string('smartmenusdynamiccoursesdaterangepast', 'theme_boost_union_phd'),
            self::RANGE_PRESENT =>
                get_string('smartmenusdynamiccoursesdaterangepresent', 'theme_boost_union_phd'),
            self::RANGE_FUTURE =>
                get_string('smartmenusdynamiccoursesdaterangefuture', 'theme_boost_union_phd'),
        ];
    }
    ...
    public static function get_listsort_options(): array {
        return [
            self::LISTSORT_FULLNAME_ASC =>
                get_string('smartmenusmenuitemlistsortfullnameasc', 'theme_boost_union_phd'),
            self::LISTSORT_FULLNAME_DESC =>
                get_string('smartmenusmenuitemlistsortfullnamedesc', 'theme_boost_union_phd'),
            self::LISTSORT_SHORTNAME_ASC =>
                get_string('smartmenusmenuitemlistsortshortnameasc', 'theme_boost_union_phd'),
            self::LISTSORT_SHORTNAME_DESC =>
                get_string('smartmenusmenuitemlistsortshortnamedesc', 'theme_boost_union_phd'),
            self::LISTSORT_COURSEID_ASC =>
                get_string('smartmenusmenuitemlistsortcourseidasc', 'theme_boost_union_phd'),
            self::LISTSORT_COURSEID_DESC =>
                get_string('smartmenusmenuitemlistsortcourseiddesc', 'theme_boost_union_phd'),
            self::LISTSORT_COURSEIDNUMBER_ASC =>
                get_string('smartmenusmenuitemlistsortcourseidnumberasc', 'theme_boost_union_phd'),
            self::LISTSORT_COURSEIDNUMBER_DESC =>
                get_string('smartmenusmenuitemlistsortcourseidnumberdesc', 'theme_boost_union_phd'),
        ];
    }
    ...
    public static function get_displayfield_options(): array {
        return [
            self::FIELD_FULLNAME => get_string('smartmenusmenuitemdisplayfieldcoursefullname', 'theme_boost_union_phd'),
            self::FIELD_SHORTNAME => get_string('smartmenusmenuitemdisplayfieldcourseshortname', 'theme_boost_union_phd'),
        ];
    }
    ...
    public static function get_mode_options(): array {
        return [
            self::MODE_INLINE => get_string('smartmenusmodeinline', 'theme_boost_union_phd'),
            self::MODE_SUBMENU => get_string('smartmenusmodesubmenu', 'theme_boost_union_phd'),
        ];
    }
    ...
    public static function get_textposition_options(): array {
        return [
            self::POSITION_BELOW =>
                get_string('smartmenusmenuitemtextpositionbelowimage', 'theme_boost_union_phd'),
            self::POSITION_OVERLAYTOP =>
                get_string('smartmenusmenuitemtextpositionoverlaytop', 'theme_boost_union_phd'),
            self::POSITION_OVERLAYBOTTOM =>
                get_string('smartmenusmenuitemtextpositionoverlaybottom', 'theme_boost_union_phd'),
        ];
    }
    ...
    public static function manage_instance($formdata) {
        ...
        // Cache for menus.
        $menucache = cache::make('theme_boost_union_phd', 'smartmenus');
        // Cache for menu items.
        $cache = cache::make('theme_boost_union_phd', 'smartmenu_items');

        if (isset($formdata->id) && $oldrecord = $DB->get_record('theme_boost_union_phd_menuitems', ['id' => $formdata->id])) {
            $itemid = $formdata->id;

            $DB->update_record('theme_boost_union_phd_menuitems', $record);
            ...
            if ($oldrecord->sortorder != $record->sortorder) {
                ...
                $decrease = "UPDATE {theme_boost_union_phd_menuitems} $operation AND id != :item AND menu=:menuid";
            ...
            \core\notification::success(get_string('smartmenusmenuitemeditsuccess', 'theme_boost_union_phd'));
        } else {
            $record->sortorder = $record->sortorder ?: 1;
            $itemid = $DB->insert_record('theme_boost_union_phd_menuitems', $record);
            // Setup the order for item.
            $sql = "UPDATE {theme_boost_union_phd_menuitems}
            ...
            \core\notification::success(get_string('smartmenusmenuitemcreatesuccess', 'theme_boost_union_phd'));
