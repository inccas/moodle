 ...
 * * Render theme_boost_union_phd/drawers instead of theme_boost/drawers template
...
$activitynavigation = get_config('theme_boost_union_phd', 'activitynavigation');
if ($activitynavigation == THEME_boost_union_phd_SETTING_SELECT_YES) {
...
if (isloggedin()) {
    ...
    if (isguestuser()) {
        $sitehomerighthandblockdrawerserverconfig = get_config('theme_boost_union_phd', 'showsitehomerighthandblockdraweronguestlogin');
    } else {
        $sitehomerighthandblockdrawerserverconfig = get_config('theme_boost_union_phd', 'showsitehomerighthandblockdraweronfirstlogin');
    }

    $isadminsettingyes = ($sitehomerighthandblockdrawerserverconfig == THEME_boost_union_phd_SETTING_SELECT_YES);
    $blockdraweropen = (get_user_preferences('drawer-open-block', $isadminsettingyes)) == true;
} else {
    $courseindexopen = false;
    $blockdraweropen = false;

    if (get_config('theme_boost_union_phd', 'showsitehomerighthandblockdraweronvisit') == THEME_boost_union_phd_SETTING_SELECT_YES) {
...
if (defined('BEHAT_SITE_RUNNING')) {
    try {
        if (
            get_config('theme_boost_union_phd', 'showsitehomerighthandblockdraweronvisit') === false &&
            get_config('theme_boost_union_phd', 'showsitehomerighthandblockdraweronguestlogin') === false &&
            get_config('theme_boost_union_phd', 'showsitehomerighthandblockdraweronfirstlogin') === false
        ) {
            $blockdraweropen = true;
        }
    } catch (Exception $e) {
...
// Render drawers.mustache from boost_union_phd.
echo $OUTPUT->render_from_template('theme_boost_union_phd/drawers', $templatecontext);
