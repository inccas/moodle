 ...
 * @return string.
 */
function theme_boost_union_phd_get_course_related_hints() {
    ...
    if (get_config('theme_boost_union_phd', 'showhintcoursehidden') == THEME_boost_union_phd_SETTING_SELECT_YES
            && has_capability('theme/boost_union_phd:viewhintinhiddencourse', \context_course::instance($COURSE->id))
            && $PAGE->has_set_url()
        ...
        $html .= $OUTPUT->render_from_template('theme_boost_union_phd/course-hint-hidden', $templatecontext);
    }
    ...
    // intended.
    if (get_config('theme_boost_union_phd', 'showhintcourseguestaccess') == THEME_boost_union_phd_SETTING_SELECT_YES
        ...
        // Render template and add it to HTML code.
        $html .= $OUTPUT->render_from_template('theme_boost_union_phd/course-hint-guestaccess', $templatecontext);
    }
    ...
    // capability "theme/boost_union_phd:viewhintcourseselfenrol".
    if (get_config('theme_boost_union_phd', 'showhintcourseselfenrol') == THEME_boost_union_phd_SETTING_SELECT_YES
            && has_capability('theme/boost_union_phd:viewhintcourseselfenrol', \context_course::instance($COURSE->id))
            && $PAGE->has_set_url()
            && $PAGE->url->compare(new moodle_url('/course/view.php'), URL_MATCH_BASE)
            && $COURSE->visible == true) {
        ...
        if (!empty($selfenrolinstances) &&
            ...
            if ($selfenrolmentpossiblecurrently == true) {
                $templatecontext['selfenrolhintstart'] = get_string('showhintcourseselfenrolstartcurrently', 'theme_boost_union_phd');
            } else if ($selfenrolmentpossiblefuture == true) {
                $templatecontext['selfenrolhintstart'] = get_string('showhintcourseselfenrolstartfuture', 'theme_boost_union_phd');
            }
            ...
            foreach ($selfenrolinstances as $selfenrolinstanceid => $selfenrolinstanceobject) {
                ...
                if ($selfenrolinstanceobject->unrestrictedness == 'unlimited') {
                    $templatecontext['selfenrolinstances'][] = get_string('showhintcourseselfenrolunlimited', 'theme_boost_union_phd',
                            array('name' => $selfenrolinstanceobject->name));
                } else if ($selfenrolinstanceobject->unrestrictedness == 'until') {
                    $templatecontext['selfenrolinstances'][] = get_string('showhintcourseselfenroluntil', 'theme_boost_union_phd',
                            array('name' => $selfenrolinstanceobject->name,
                                    'until' => userdate($selfenrolinstanceobject->enddate)));
                } else if ($selfenrolinstanceobject->unrestrictedness == 'from') {
                    $templatecontext['selfenrolinstances'][] = get_string('showhintcourseselfenrolfrom', 'theme_boost_union_phd',
                            array('name' => $selfenrolinstanceobject->name,
                                    'from' => userdate($selfenrolinstanceobject->startdate)));
                } else if ($selfenrolinstanceobject->unrestrictedness == 'since') {
                    $templatecontext['selfenrolinstances'][] = get_string('showhintcourseselfenrolsince', 'theme_boost_union_phd',
                            array('name' => $selfenrolinstanceobject->name,
                                    'since' => userdate($selfenrolinstanceobject->startdate)));
                } else if ($selfenrolinstanceobject->unrestrictedness == 'fromuntil') {
                    $templatecontext['selfenrolinstances'][] = get_string('showhintcourseselfenrolfromuntil', 'theme_boost_union_phd',
                            array('name' => $selfenrolinstanceobject->name,
                                    'until' => userdate($selfenrolinstanceobject->enddate),
                                    'from' => userdate($selfenrolinstanceobject->startdate)));
                } else if ($selfenrolinstanceobject->unrestrictedness == 'sinceuntil') {
                    $templatecontext['selfenrolinstances'][] = get_string('showhintcourseselfenrolsinceuntil', 'theme_boost_union_phd',
                            array('name' => $selfenrolinstanceobject->name,
                                    'until' => userdate($selfenrolinstanceobject->enddate),
                                    'since' => userdate($selfenrolinstanceobject->startdate)));
                }
            ...
            // Render template and add it to HTML code.
            $html .= $OUTPUT->render_from_template('theme_boost_union_phd/course-hint-selfenrol', $templatecontext);
        }
    }
    ...
    if (get_config('theme_boost_union_phd', 'showswitchedroleincourse') === THEME_boost_union_phd_SETTING_SELECT_YES
            && is_role_switched($COURSE->id) ) {
        ...
        // Render template and add it to HTML code.
        $html .= $OUTPUT->render_from_template('theme_boost_union_phd/course-hint-switchedrole', $templatecontext);
    }
 ...
 * @return string.
 */
function theme_boost_union_phd_get_staticpage_link($page) {
    // Compose the URL object.
    $url = new moodle_url('/theme/boost_union_phd/pages/'.$page.'.php');
 ...
 * @return string.
 */
function theme_boost_union_phd_get_staticpage_pagetitle($page) {
    // Get the configured page title.
    $pagetitleconfig = format_string(get_config('theme_boost_union_phd', $page.'pagetitle'), true,
    ...
    if ($pagetitleconfig) {
        ...
        return get_string($page.'pagetitledefault', 'theme_boost_union_phd');
    }
}
 ...
 * @return boolean.
 */
function theme_boost_union_phd_infobanner_is_shown_on_page($bannerno) {
    global $PAGE;

    // Get theme config.
    $config = get_config('theme_boost_union_phd');

    // If the info banner is enabled.
    $enabledsettingname = 'infobanner'.$bannerno.'enabled';
    if ($config->{$enabledsettingname} == THEME_boost_union_phd_SETTING_SELECT_YES) {
        ...
        if (!empty($config->{$contentsettingname})) {
            ...
            if ($showonpage == true) {
                ...
                if ($config->{$modesettingname} == THEME_boost_union_phd_SETTING_INFOBANNERMODE_TIMEBASED) {
                    ...
                    if (get_user_preferences('theme_boost_union_phd_infobanner'.$bannerno.'_dismissed') != true) {
                        return true;
                    }
                }
            }
 ...
 * @return boolean.
 */
function theme_boost_union_phd_compare_order($a, $b) {
 ...
 */
function theme_boost_union_phd_infobanner_reset_visibility($no) {
    global $DB;
    ...
    $params = ['name' => 'theme_boost_union_phd_infobanner'.$no.'_dismissed', 'value' => '1'];
    ...
    foreach ($users as $user) {
        try {
            unset_user_preference('theme_boost_union_phd_infobanner'.$no.'_dismissed', $user->userid);
 ...
 */
function theme_boost_union_phd_get_random_loginbackgroundimage_number() {
    // Static variable.
    static $number = null;

    if ($number == null) {
        // Get all files for loginbackgroundimages.
        $files = theme_boost_union_phd_get_loginbackgroundimage_files();
 ...
 * @return string
 */
function theme_boost_union_phd_get_random_loginbackgroundimage_class() {
    // Get the static random number.
    $number = theme_boost_union_phd_get_random_loginbackgroundimage_number();
 ...
 * @throws dml_exception
 */
function theme_boost_union_phd_get_loginbackgroundimage_files() {
    ...
    if ($files == null) {
        ...
        $files = $fs->get_area_files($systemcontext->id, 'theme_boost_union_phd', 'loginbackgroundimage',
                false, 'itemid', false);
    }
 ...
 * @return string|null
 */
function theme_boost_union_phd_get_urloftilebackgroundimage($tileno) {
    ...
    if ($tileno < 0 || $tileno > THEME_boost_union_phd_SETTING_ADVERTISEMENTTILES_COUNT) {
        return null;
    }

    // Get the background image config for this tile.
    $bgconfig = get_config('theme_boost_union_phd', 'tile'.$tileno.'backgroundimage');
    ...
    if (!empty($bgconfig)) {
        ...
        // Get all files from filearea.
        $files = $fs->get_area_files($systemcontext->id, 'theme_boost_union_phd', 'tilebackgroundimage'.$tileno,
                false, 'itemid', false);
 ...
 * @return string
 */
function theme_boost_union_phd_get_loginbackgroundimage_scss() {
 ...
 * @throws dml_exception
 */
function theme_boost_union_phd_get_loginbackgroundimage_text() {
    ...
    if ($number != null) {
        ...
        $lines = explode("\n", get_config('theme_boost_union_phd', 'loginbackgroundimagetext'));
 ...
 * @throws dml_exception
 */
function theme_boost_union_phd_get_additionalresources_templatecontext() {
    global $OUTPUT;
    ...
    if ($filesforcontext == null) {
        ...
        // Get all files from filearea.
        $files = $fs->get_area_files($systemcontext->id, 'theme_boost_union_phd', 'additionalresources', false, 'itemid', false);

        // Iterate over the files and fill the templatecontext of the file list.
        $filesforcontext = array();
        foreach ($files as $af) {
            $urlpersistent = new moodle_url('/pluginfile.php/1/theme_boost_union_phd/additionalresources/0/'.$af->get_filename());
            $urlrevisioned = new moodle_url('/pluginfile.php/1/theme_boost_union_phd/additionalresources/'.theme_get_revision().
                    '/'.$af->get_filename());
 ...
 * @throws dml_exception
 */
function theme_boost_union_phd_get_customfonts_templatecontext() {
    global $OUTPUT;
    ...
    if ($filesforcontext == null) {
        ...
        // Get all files from filearea.
        $files = $fs->get_area_files($systemcontext->id, 'theme_boost_union_phd', 'customfonts', false, 'itemid', false);

        // Get the webfonts extensions list.
        $webfonts = theme_boost_union_phd_get_webfonts_extensions();
        ...
        foreach ($files as $af) {
            ...
            $urlpersistent = new moodle_url('/pluginfile.php/1/theme_boost_union_phd/customfonts/0/'.$filename);
 ...
 * @return array
 */
function theme_boost_union_phd_get_webfonts_extensions() {
 ...
 * @throws coding_exception
 */
function theme_boost_union_phd_register_webfonts_filetypes() {
    global $CFG;
 ...
 * @return string|null
 */
function theme_boost_union_phd_get_emailbrandinghtmlpreview() {
    global $OUTPUT;

    // Get branding snippets.
    $htmlprefix = get_string('templateemailhtmlprefix', 'theme_boost_union_phd');
    $htmlsuffix = get_string('templateemailhtmlsuffix', 'theme_boost_union_phd');
    ...
    $mailtemplatecontext = array('body' => get_string('emailbrandinghtmldemobody', 'theme_boost_union_phd'));
    ...
    $preview = $OUTPUT->render_from_template('theme_boost_union_phd/emailpreview', $previewtemplatecontext);
 ...
 * @return string|null
 */
function theme_boost_union_phd_get_emailbrandingtextpreview() {
    global $OUTPUT;

    // Get branding snippets.
    $textprefix = get_string('templateemailtextprefix', 'theme_boost_union_phd');
    $textsuffix = get_string('templateemailtextsuffix', 'theme_boost_union_phd');
    ...
    // Otherwise, compose mail text.
    $mailtemplatecontext = array('body' => get_string('emailbrandingtextdemobody', 'theme_boost_union_phd'));
 ...
 * @return void
 */
function theme_boost_union_phd_fontawesome_checkin() {
    // Create cache for FontAwesome files.
    $cache = cache::make('theme_boost_union_phd', 'fontawesome');
    ...
    if ($faconfig != THEME_boost_union_phd_SETTING_FAVERSION_NONE && $faconfig != null) {
        ...
        $filestructure = theme_boost_union_phd_get_fontawesome_filestructure($faconfig);
        ...
        if ($filestructure != null) {
            ...
            foreach ($filestructure as $folder => $files) {
                ...
                foreach ($files as $file => $expected) {
                    ...
                    $fsfile = $fs->get_file($systemcontext->id, 'theme_boost_union_phd', 'fontawesome', 0, '/'.$folder.'/', $file);
 ...
 * @return array
 */
function theme_boost_union_phd_get_fontawesome_extensions() {
 ...
 *
 * @param string $version The FontAwesome version, given as THEME_boost_union_phd_SETTING_FAVERSION_* constant.
 *
 * @return array|null The array of files or null if an invalid FontAwesome version was provided.
 */
function theme_boost_union_phd_get_fontawesome_filestructure($version) {
    // Pick the files for the selected FA version.
    switch ($version) {
        case THEME_boost_union_phd_SETTING_FAVERSION_FA6FREE:
            $files = array('css' => array('fontawesome.min.css' => THEME_boost_union_phd_SETTING_FAFILES_MANDATORY,
                            'solid.min.css' => THEME_boost_union_phd_SETTING_FAFILES_MANDATORY,
                            'regular.min.css' => THEME_boost_union_phd_SETTING_FAFILES_OPTIONAL,
                            'brands.min.css' => THEME_boost_union_phd_SETTING_FAFILES_OPTIONAL,
                            'v4-font-face.min.css' => THEME_boost_union_phd_SETTING_FAFILES_MANDATORY),
                    'webfonts' => array('fa-solid-900.woff2' => THEME_boost_union_phd_SETTING_FAFILES_MANDATORY,
                            'fa-solid-900.ttf' => THEME_boost_union_phd_SETTING_FAFILES_MANDATORY,
                            'fa-regular-400.woff2' => THEME_boost_union_phd_SETTING_FAFILES_OPTIONAL,
                            'fa-regular-400.ttf' => THEME_boost_union_phd_SETTING_FAFILES_OPTIONAL,
                            'fa-brands-400.woff2' => THEME_boost_union_phd_SETTING_FAFILES_OPTIONAL,
                            'fa-brands-400.ttf' => THEME_boost_union_phd_SETTING_FAFILES_OPTIONAL,
                            'fa-v4compatibility.woff2' => THEME_boost_union_phd_SETTING_FAFILES_MANDATORY,
                            'fa-v4compatibility.ttf' => THEME_boost_union_phd_SETTING_FAFILES_MANDATORY));
            break;
        default:
 ...
 * This means it uses the same data source as the theme_boost_union_phd_add_fontawesome_to_page() function which adds
 ...
 * @throws dml_exception
 */
function theme_boost_union_phd_get_fontawesome_templatecontext() {
    // Create cache for FontAwesome files.
    $cache = cache::make('theme_boost_union_phd', 'fontawesome');

    // If the cache is completely empty, check the files in on-the-fly.
    if ($cache->get('checkedin') != true) {
        theme_boost_union_phd_fontawesome_checkin();
    }
        ...
        if ($filestructure != null) {
            ...
            foreach ($filestructure as $folder => $files) {
                ...
                foreach ($files as $file => $expected) {
                    ...
                    if ($expected == THEME_boost_union_phd_SETTING_FAFILES_MANDATORY) {
                        $mandatory = true;
                    } else {
                        $mandatory = false;
                    }
                    ...
                    $description = get_string('fontawesomelistfileinfo-' . $faconfig . '-' . $fileidentifier, 'theme_boost_union_phd');
 ...
 */
function theme_boost_union_phd_get_fontawesome_checks_templatecontext() {
    global $CFG;

    // Get FontAwesome version config.
    $version = get_config('theme_boost_union_phd', 'fontawesomeversion');
        ...
        case THEME_boost_union_phd_SETTING_FAVERSION_FA6FREE:
            $checks = array(
                    array('icon' => '<i class="fa fa-check-circle-o fa-3x fa-fw"></i>',
                            'title' => get_string('fontawesomecheck-fa6free-general-title', 'theme_boost_union_phd'),
                            'description' => get_string('fontawesomecheck-fa6free-general-description', 'theme_boost_union_phd')),
                    array('icon' => '<i class="fa fa-map-o fa-3x fa-fw"></i>',
                            'title' => get_string('fontawesomecheck-fa6free-fallback-title', 'theme_boost_union_phd'),
                            'description' => get_string('fontawesomecheck-fa6free-fallback-description', 'theme_boost_union_phd')),
                    array('icon' => '<i class="fa-solid fa-virus-covid fa-3x fa-fw"></i>',
                            'title' => get_string('fontawesomecheck-fa6free-newstuff-title', 'theme_boost_union_phd'),
                            'description' => get_string('fontawesomecheck-fa6free-newstuff-description', 'theme_boost_union_phd')),
            );
            break;
        default:
            // This only happens if an invalid version was provided.
    ...
    if (file_exists($CFG->dirroot.'/filter/fontawesome/version.php')) {
        $checks[] = array('icon' => format_text('[fa-solid fa-users-line fa-3x fa-fw]'),
                'title' => get_string('fontawesomecheck-fa6free-filter-title', 'theme_boost_union_phd'),
                'description' => get_string('fontawesomecheck-fa6free-filter-description', 'theme_boost_union_phd'));
    }
 ...
 * @return string
 */
function theme_boost_union_phd_get_externaladminpage_title($pagename) {
    global $SITE;
    ...
    $title .= get_string('pluginname', 'theme_boost_union_phd').': ';
    $title .= $pagename;
 ...
 * @return string
 */
function theme_boost_union_phd_get_externaladminpage_heading() {
    global $SITE;
 ...
 * This function uses the fontawesome cache definition, i.e. it does not load the files from the filearea directly.
 * It's meant to be called by theme_boost_union_phd_before_standard_html_head() only.
 * *
 * @throws coding_exception
 * @throws dml_exception
 * @throws moodle_exception
 */
function theme_boost_union_phd_add_fontawesome_to_page() {
    global $PAGE;
    ...
    if ($faconfig != THEME_boost_union_phd_SETTING_FAVERSION_NONE && $faconfig != null) {
        ...
        foreach ($cachedfolder as $cachedfile) {
            ...
            $facssurl = new moodle_url('/pluginfile.php/1/theme_boost_union_phd/fontawesome/' .
 ...
 * @throws moodle_exception
 */
function theme_boost_union_phd_add_flavourcss_to_page() {
    global $CFG, $PAGE;

    // Require flavours library.
    require_once($CFG->dirroot . '/theme/boost_union_phd/flavours/flavourslib.php');

    // If any flavour applies to this page.
    $flavour = theme_boost_union_phd_get_flavour_which_applies();
    if ($flavour != null) {
        // Build the flavour CSS file URL.
        $flavourcssurl = new moodle_url('/theme/boost_union_phd/flavours/styles.php',
 ...
 * @return null | string
 */
function theme_boost_union_phd_get_course_header_image_url() {
    global $PAGE;
    ...
    if ($courseimage) {
    ...
    } else if (get_config('theme_boost_union_phd', 'courseheaderimagefallback')) {
        // Get the system context.
        $systemcontext = \context_system::instance();
        ...
        // Get all files from filearea.
        $files = $fs->get_area_files($systemcontext->id, 'theme_boost_union_phd', 'courseheaderimagefallback',
            false, 'itemid', false);
 ...
 * @throws moodle_exception
 */
function theme_boost_union_phd_set_mobilecss_url() {
    // Check if the admin has set any CSS code for the Mobile app.
    $csscode = get_config('theme_boost_union_phd', 'mobilescss');
    if (!empty($csscode)) {
        ...
        $mobilescssurl = new moodle_url('/theme/boost_union_phd/mobile/styles.php', array('rev' => time()));
 ...
 */
function theme_boost_union_phd_get_additional_regions($pageregions=[]) {
    $regions = [
 ...
 * @return array $regions
 */
function theme_boost_union_phd_get_block_regions($layout) {

    // Get the admin setting for the layout.
    $regionsettings = get_config('theme_boost_union_phd', 'blockregionsfor'.$layout);
 ...
 */
function theme_boost_union_phd_rrmdir($src , $del) {
    ...
    while (false !== ($file = readdir($dir))) {
        ...
        if (($file != '.') && ($file != '..')) {
            ...
            if (is_dir($full)) {
                ...
                theme_boost_union_phd_rrmdir($full, true);
 ...
 */
function theme_boost_union_phd_check_mod_icons_cleanup() {
    global $CFG;

    // Get the modiconsenable setting.
    $modiconsenable = get_config('theme_boost_union_phd', 'modiconsenable');

    // If modiconsenable was just enabled, return directly as everything is fine.
    if ($modiconsenable != THEME_boost_union_phd_SETTING_SELECT_NO) {
        return;
    }
    ...
    if (is_dir($pixpluginpath)) {
        theme_boost_union_phd_rrmdir($pixpluginpath, true);
    }
 ...
 * @throws moodle_exception
 */
function theme_boost_union_phd_place_mod_icons() {
    global $CFG, $DB;
    ...
    if ($modiconsenable != THEME_boost_union_phd_SETTING_SELECT_YES) {
        return;
    }
    ...
    $pixpluginpath = $CFG->dataroot.DIRECTORY_SEPARATOR.'pix_plugins'.DIRECTORY_SEPARATOR.'mod';
    if (is_dir($pixpluginpath)) {
        theme_boost_union_phd_rrmdir($pixpluginpath, false);
    }
    ...
    // Get all files from filearea.
    $files = $fs->get_area_files($systemcontext->id, 'theme_boost_union_phd', 'modicons', false, 'itemid', true);
 ...
 * @throws dml_exception
 */
function theme_boost_union_phd_get_modicon_templatecontext () {
    global $DB;
    ...
    // Get all files from filearea.
    $files = $fs->get_area_files($systemcontext->id, 'theme_boost_union_phd', 'modicons', false, 'filepath,filename', true);
 ...
 * @return string
 */
function theme_boost_union_phd_get_scss_to_mark_external_links($theme) {
    global $CFG;
    ...
    if ($theme->settings->markexternallinks == THEME_boost_union_phd_SETTING_SELECT_YES) {
