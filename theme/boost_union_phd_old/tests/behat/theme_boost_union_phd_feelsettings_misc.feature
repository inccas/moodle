@theme @theme_boost_union_phd @theme_boost_union_phd_feelsetttings @theme_boost_union_phd_feelsetttings_misc
Feature: Configuring the theme_boost_union_phd plugin for the "Miscellaneous" tab on the "Feel" page
  ...
  Scenario: Setting: JavaScript disabled hint - Enable the setting and make sure the hint is shown when JavaScript is disabled
    Given the following config values are set as admin:
      | config                 | value | plugin            |
      | javascriptdisabledhint | yes   | theme_boost_union_phd |
  ...
  Scenario: Setting: JavaScript disabled hint - Disable the setting and make sure the hint is not shown when JavaScript is disabled
    Given the following config values are set as admin:
      | config                 | value | plugin            |
      | javascriptdisabledhint | no    | theme_boost_union_phd |
    When I log in as "admin"
    Then "#javascriptdisabledhint" "css_element" should not exist
