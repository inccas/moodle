...
namespace theme_boost_union_ida;
...
class smartmenu {
    ...
    public static function instance($menu) {
        ...
        if (!is_array($menu) && !is_object($menu)) {
            throw new moodle_exception('menuformatnotcorrect', 'theme_boost_union_ida');
        }
    ...
    public function __construct($menu) {
        ...
        // Cache for menu.
        $this->cache = cache::make('theme_boost_union_ida', 'smartmenus');
    }
    ...
    public function delete_menu() {
        global $DB;
        if ($DB->delete_records('theme_boost_union_ida_menus', ['id' => $this->id])) {
            // Delete all its items.
            $DB->delete_records('theme_boost_union_ida_menuitems', ['menu' => $this->id]);
    ...
    public function move_upward() {
        ...
        if ($currentposition > 1) {
            $menus = $DB->get_records('theme_boost_union_ida_menus', ['sortorder' => $currentposition - 1]);
            if (empty($menus)) {
                return false;
            }
            $prevmenu = current($menus);
            // Update the menu position to upwards.
            $DB->set_field('theme_boost_union_ida_menus', 'sortorder', $prevmenu->sortorder, ['id' => $this->id]);
            // Set the prevmenu position to down.
            $DB->set_field('theme_boost_union_ida_menus', 'sortorder', $currentposition, ['id' => $prevmenu->id]);
    ...
    public function move_downward() {
        ...
        if ($currentposition < self::get_menuscount()) {
            ...
            $DB->set_field('theme_boost_union_ida_menus', 'sortorder', $nextmenu->sortorder, ['id' => $this->id]);
            // Set the nextmenu position to up.
            $DB->set_field('theme_boost_union_ida_menus', 'sortorder', $currentposition, ['id' => $nextmenu->id]);
            // Purge the menus list, need to recreate in new order.
            $this->cache->delete(self::CACHE_MENUSLIST);
    ...
    public function duplicate() {
        ...
        if (!empty($items)) {
            ...
            foreach ($items as $item) {
                $item = \theme_boost_union_ida\smartmenu_item::instance($item->id)->item;
                $item->id = 0;
                // New menu id as menu for duplicate item.
                $item->menu = $duplicateid;
                // Create the item.
                \theme_boost_union_ida\smartmenu_item::manage_instance($item);
            }
        }
        // Success message for duplicated menu.
        \core\notification::success(get_string('smartmenusmenuduplicatesuccess', 'theme_boost_union_ida'));
    ...
    public function update_field($key, $value) {
        ...
        $result = $DB->set_field('theme_boost_union_ida_menus', $key, $value, ['id' => $this->id]);
    ...
    public function get_menu_items() {
        ...
        $sql = "SELECT mi.* FROM {theme_boost_union_ida_menuitems} mi
            LEFT JOIN {theme_boost_union_ida_menus} mn ON mn.id = mi.menu
            WHERE mi.menu=:id
    ...
    public function build($resetcache=false) {
        ...
        if (empty($itemcache)) {
            $itemcache = cache::make('theme_boost_union_ida', 'smartmenu_items');
        }
        ...
        if (!empty($menuitems)) {
            ...
            foreach ($menuitems as $item) {
                ...
                $item = \theme_boost_union_ida\smartmenu_item::instance($item, $this->menu)->build();
    ...
    public static function get_menu($id) {
        ...
        if ($record = $DB->get_record('theme_boost_union_ida_menus', ['id' => $id])) {
            ...
            throw new moodle_exception('error:smartmenusmenunotfound', 'theme_boost_union_ida');
        }
        return false;
    }
    ...
    public static function get_menuscount() {
        global $DB;
        return $DB->count_records('theme_boost_union_ida_menus', []);
    }
    ...
    public static function get_lastmenu() {
        global $DB;
        $records = $DB->get_records_sql('SELECT * FROM {theme_boost_union_ida_menus} ORDER BY sortorder DESC', [], 0, 1);
    ...
    public static function get_locations() {
        ...
        $locations = [
            self::LOCATION_MAIN => get_string('smartmenusmenulocationmain', 'theme_boost_union_ida'),
            self::LOCATION_MENU => get_string('smartmenusmenulocationmenu', 'theme_boost_union_ida'),
            self::LOCATION_USER => get_string('smartmenusmenulocationuser', 'theme_boost_union_ida'),
            self::LOCATION_BOTTOM => get_string('smartmenusmenulocationbottom', 'theme_boost_union_ida'),
        ];
    ...
    public static function get_types() {
        $types = [
            self::TYPE_LIST => get_string('smartmenusmenutypelist', 'theme_boost_union_ida'),
            self::TYPE_CARD => get_string('smartmenusmenutypecard', 'theme_boost_union_ida'),
        ];
    ...
    public static function get_mode_options(): array {
        return [
            self::MODE_SUBMENU => get_string('smartmenusmodesubmenu', 'theme_boost_union_ida'),
            self::MODE_INLINE => get_string('smartmenusmodeinline', 'theme_boost_union_ida'),
        ];
    }
    ...
    public static function get_showdescription_options(): array {
        return [
            self::DESC_NEVER => get_string('smartmenusmenushowdescriptionnever', 'theme_boost_union_ida'),
            self::DESC_ABOVE => get_string('smartmenusmenushowdescriptionabove', 'theme_boost_union_ida'),
            self::DESC_BELOW => get_string('smartmenusmenushowdescriptionbelow', 'theme_boost_union_ida'),
            self::DESC_HELP => get_string('smartmenusmenushowdescriptionhelp', 'theme_boost_union_ida'),
        ];
    }
    ...
    public static function get_moremenu_options(): array {
        return [
            self::MOREMENU_DONOTCHANGE => get_string('dontchange', 'theme_boost_union_ida'),
            self::MOREMENU_INTO => get_string('smartmenusmenumoremenubehaviorforceinto', 'theme_boost_union_ida'),
            self::MOREMENU_OUTSIDE => get_string('smartmenusmenumoremenubehaviorkeepoutside', 'theme_boost_union_ida'),
        ];
    }
    ...
    public static function get_cardsize_options(): array {
        return [
            self::CARDSIZE_TINY => get_string('smartmenusmenucardsizetiny', 'theme_boost_union_ida').' (50px)',
            self::CARDSIZE_SMALL => get_string('smartmenusmenucardsizesmall', 'theme_boost_union_ida').' (100px)',
            self::CARDSIZE_MEDIUM => get_string('smartmenusmenucardsizemedium', 'theme_boost_union_ida').' (150px)',
            self::CARDSIZE_LARGE => get_string('smartmenusmenucardsizelarge', 'theme_boost_union_ida').' (200px)',
        ];
    }
    ...
    public static function get_cardform_options(): array {
        return[
            self::CARDFORM_SQUARE =>
                get_string('smartmenusmenucardformsquare', 'theme_boost_union_ida').' (1/1)',
            self::CARDFORM_PORTRAIT =>
                get_string('smartmenusmenucardformportrait', 'theme_boost_union_ida').' (2/3)',
            self::CARDFORM_LANDSCAPE =>
                get_string('smartmenusmenucardformlandscape', 'theme_boost_union_ida').' (3/2)',
            self::CARDFORM_FULLWIDTH =>
                get_string('smartmenusmenucardformfullwidth', 'theme_boost_union_ida'),
        ];
    }
    ...
    public static function get_cardoverflowbehaviour_options(): array {
        return [
            self::CARDOVERFLOWBEHAVIOUR_NOWRAP =>
                get_string('smartmenusmenucardoverflowbehaviornowrap', 'theme_boost_union_ida'),
            self::CARDOVERFLOWBEHAVIOUR_WRAP =>
                get_string('smartmenusmenucardoverflowbehaviorwrap', 'theme_boost_union_ida'),
        ];
    }
    ...
    public static function get_byadmin_options(): array {
        return [
            self::BYADMIN_ALL => get_string('smartmenusbyadmin_all', 'theme_boost_union_ida'),
            self::BYADMIN_ADMINS => get_string('smartmenusbyadmin_admins', 'theme_boost_union_ida'),
            self::BYADMIN_NONADMINS => get_string('smartmenusbyadmin_nonadmins', 'theme_boost_union_ida'),
        ];
    }
    ...
    public static function manage_instance($formdata) {
        ...
        $cache = cache::make('theme_boost_union_ida', 'smartmenus');

        $transaction = $DB->start_delegated_transaction();

        if (isset($formdata->id) && $DB->record_exists('theme_boost_union_ida_menus', ['id' => $formdata->id])) {
            $menuid = $formdata->id;

            $DB->update_record('theme_boost_union_ida_menus', $record);
            ...
            \core\notification::success(get_string('smartmenusmenueditsuccess', 'theme_boost_union_ida'));
        } else {
            // Setup the menu order.
            $lastmenu = self::get_lastmenu();
            $record->sortorder = isset($lastmenu->sortorder) ? $lastmenu->sortorder + 1 : 1;
            $menuid = $DB->insert_record('theme_boost_union_ida_menus', $record);
            // New menu added, recreate the menuslist.
            $cache->delete(self::CACHE_MENUSLIST);
            // Show the menu inserted success notification.
            \core\notification::success(get_string('smartmenusmenucreatesuccess', 'theme_boost_union_ida'));
        }
    ...
    public static function get_menus() {
        ...
        $topmenus = $DB->get_records('theme_boost_union_ida_menus', [], 'sortorder ASC');
        return $topmenus;
    }
    ...
    public static function build_smartmenu() {
        ...
        $removecache = (get_user_preferences('theme_boost_union_ida_menu_purgesessioncache', false) == true);
