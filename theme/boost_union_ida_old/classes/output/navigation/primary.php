...
use renderer_base;
use custom_menu;
use theme_boost_union_ida\smartmenu;
...
class primary extends \core\navigation\output\primary {
    ...
    public function export_for_template(?renderer_base $output = null): array {
        ...
        if (!$cache->get(smartmenu::CACHE_MENUSLIST)) {
            ...
            if (!$dbman->table_exists('theme_boost_union_ida_menus')) {
    ...
    public function get_user_menu(renderer_base $output): array {
        ...
        // in the future).
        $addpreferredlangsetting = get_config('theme_boost_union_ida', 'addpreferredlang');
        if (!isset($addpreferredlangsetting) || $addpreferredlangsetting == THEME_BOOST_UNION_SETTING_SELECT_NO) {
            ...
            if ($addpreferredlangsetting == THEME_BOOST_UNION_IDA_SETTING_SELECT_YES &&
                ...
                foreach ($parentoutput['submenus'] as $sm) {
                    ...
                    if ($sm->title == $needle) {
                        ...
                        $spfnode = [
                            'title' => get_string('setpreferredlanglink', 'theme_boost_union_ida'),
                            'text' => get_string('setpreferredlanglink', 'theme_boost_union_ida'),
                            'link' => true,
                            'isactive' => false,
