...
if (isloggedin()) {
    ...
    if (isguestuser()) {
        $sitehomerighthandblockdrawerserverconfig = get_config('theme_boost_union_ida', 'showsitehomerighthandblockdraweronguestlogin');
    } else {
...
if (defined('BEHAT_SITE_RUNNING') && get_user_preferences('behat_keep_drawer_closed') != 1) {
    try {
        if (
            get_config('theme_boost_union_ida', 'showsitehomerighthandblockdraweronvisit') === false &&
            get_config('theme_boost_union_ida', 'showsitehomerighthandblockdraweronguestlogin') === false &&
            get_config('theme_boost_union_ida', 'showsitehomerighthandblockdraweronfirstlogin') === false
        ) {
            $blockdraweropen = true;
        }
    } catch (Exception $e) {
...
// Load the navigation from boost_union_ida primary navigation, the extended version of core primary navigation.
// It includes the smart menus and menu items, for multiple locations.
$primary = new theme_boost_union_ida\output\navigation\primary($PAGE);
...
// Render drawers.mustache from theme_boost (which is overridden in theme_boost_union_ida).
