...
/**
 * Theme Boost Union Dhoch3 - Static pages layout include.
 *
...
foreach ($staticpages as $staticpage) {
    ...
    if (isset($config->{'enable'.$staticpage}) &&
            $config->{'enable'.$staticpage} == THEME_BOOST_UNION_DHOCH3_SETTING_SELECT_YES) {
        // If the admin wants to show a link in the footnote or in both locations.
        if ($config->{$staticpage.'linkposition'} == THEME_BOOST_UNION_DHOCH3_SETTING_STATICPAGELINKPOSITION_FOOTNOTE ||
                $config->{$staticpage.'linkposition'} == THEME_BOOST_UNION_DHOCH3_SETTING_STATICPAGELINKPOSITION_BOTH) {
        ...
        if ($config->{$staticpage.'linkposition'} == THEME_BOOST_UNION_DHOCH3_SETTING_STATICPAGELINKPOSITION_FOOTER ||
                $config->{$staticpage.'linkposition'} == THEME_BOOST_UNION_DHOCH3_SETTING_STATICPAGELINKPOSITION_BOTH) {
        ...
        $templatecontext[$staticpage.'link'] = theme_boost_union_dhoch3_get_staticpage_link($staticpage);
        $templatecontext[$staticpage.'pagetitle'] = theme_boost_union_dhoch3_get_staticpage_pagetitle($staticpage);
    }
}
