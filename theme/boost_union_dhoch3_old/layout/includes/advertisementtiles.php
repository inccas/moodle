...
/**
 * Theme Boost Union Dhoch3 - advertisement tiles layout include.
 *
...
for ($i = 1; $i <= THEME_BOOST_UNION_DHOCH3_SETTING_ADVERTISEMENTTILES_COUNT; $i++) {
    ...
    if (isset($config->{'tile'.$i.'enabled'}) &&
            $config->{'tile'.$i.'enabled'} == THEME_BOOST_UNION_DHOCH3_SETTING_SELECT_YES) {
        ...
        if ($config->{'tile'.$i.'linktarget'} == THEME_BOOST_UNION_DHOCH3_SETTING_LINKTARGET_NEWTAB) {
        ...
        $bgimage = theme_boost_union_dhoch3_get_urloftilebackgroundimage($i);
        ...
        switch ($config->{'tile'.$i.'contentstyle'}) {
            case THEME_BOOST_UNION_DHOCH3_SETTING_CONTENTSTYLE_NOCHANGE:
                $contentstyleclass = '';
                break;
...
if ($templatecontext['showadvtiles'] == true) {
    ...
    switch ($config->{'tilefrontpageposition'}) {
        case THEME_BOOST_UNION_DHOCH3_SETTING_ADVERTISEMENTTILES_FRONTPAGEPOSITION_BEFORE:
            $templatecontext['advtilespositionbefore'] = true;
            $templatecontext['advtilespositionafter'] = false;
            break;
        case THEME_BOOST_UNION_DHOCH3_SETTING_ADVERTISEMENTTILES_FRONTPAGEPOSITION_AFTER:
    ...
    usort($advertisementtiles, 'theme_boost_union_dhoch3_compare_order');
