...
/**
 * Theme Boost Union Dhoch3 - Additional regions.
 *
...
class additionalregions {
    ...
    /**
     * Constructor.
     * Get the all regions in theme_boost_union_dhoch3_get_additional_regions function.
     */
    public function __construct() {
        global $PAGE;

        $pageregions = $PAGE->blocks->get_regions();
        $this->regions = theme_boost_union_dhoch3_get_additional_regions($pageregions);
    }
    ...
    public function regionsdata() {
        ...
        foreach ($this->regions as $name => $region) {
            ...
            if (!has_capability('theme/boost_union_dhoch3:viewregion'.$name, $PAGE->context)) {
                $regionsdata[$name] = ['hasblocks' => false];
                continue;
            }

            $regionhtml = $OUTPUT->blocks($region);
            $blockbutton = (has_capability('theme/boost_union_dhoch3:editregion'.$name, $PAGE->context)) ?
    ...
    public function export_for_template() {
        ...
        return [
            ...
            'outsideregionsplacement' => 'main-inner-outside-'.get_config('theme_boost_union_dhoch3', 'outsideregionsplacement'),
            'outsidebottomwidth' => 'theme-block-region-outside-'.get_config('theme_boost_union_dhoch3', 'blockregionoutsidebottomwidth'),
            'outsidetopwidth' => 'theme-block-region-outside-'.get_config('theme_boost_union_dhoch3', 'blockregionoutsidetopwidth'),
            'footerwidth' => 'theme-block-region-footer-'.get_config('theme_boost_union_dhoch3', 'blockregionfooterwidth'),
        ];
    }
}
