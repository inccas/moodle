...
/**
 * Theme Boost Union Dhoch3 - info banners layout include.
 *
...
for ($i = 1; $i <= THEME_BOOST_UNION_DHOCH3_SETTING_INFOBANNER_COUNT; $i++) {
    ...
    if (theme_boost_union_dhoch3_infobanner_is_shown_on_page($i)) {
        ...
        if ($PAGE->pagelayout != 'login') {
            ...
            if ($config->{$dismissiblesettingname} == THEME_BOOST_UNION_DHOCH3_SETTING_SELECT_YES) {
                $dismissible = true;
            } else {
                $dismissible = false;
            }
...
usort($infobanners, 'theme_boost_union_dhoch3_compare_order');
...
if ($dismissibleamdneeded == true) {
    $PAGE->requires->js_call_amd('theme_boost_union_dhoch3/infobanner', 'init');
}
