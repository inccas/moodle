@theme @theme_boost_union_dhoch3 @theme_boost_union_dhoch3_functionalitysettings @theme_boost_union_dhoch3_functionalitysettings_administration
Feature: Configuring the theme_boost_union_dhoch3 plugin for the "Administration" tab on the "Functionality" page
  ...
  Scenario Outline: Setting: Show view course icon - Disable and enable the setting
    Given the following config values are set as admin:
      | config                         | value   | plugin            |
      | showviewcourseiconincoursemgnt | <value> | theme_boost_union_dhoch3 |
