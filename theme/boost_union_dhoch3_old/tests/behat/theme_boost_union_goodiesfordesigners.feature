@theme @theme_boost_union_dhoch3 @theme_boost_union_dhoch3_goodiesfordesigners
Feature: Using the goodies for designers in the theme_boost_union_dhoch3 plugin
  ...
  Scenario: Feature: Mark external links
    And the following config values are set as admin:
      | config            | value                                                                         | plugin            |
      | markexternallinks | no                                                                            | theme_boost_union_dhoch3 |
      | footnote          | <a href="https://www.externallink.com" class="externallink">External Link</a> | theme_boost_union_dhoch3 |
  ...
  Scenario: Feature: Mark mailto links
    And the following config values are set as admin:
      | config          | value                                                          | plugin            |
      | markmailtolinks | no                                                             | theme_boost_union_dhoch3 |
      | footnote        | <a href="mailto:test@test.de" class="mailtolink">Mail-Link</a> | theme_boost_union_dhoch3 |
    When I log in as "admin"
    And I follow "Dashboard"
