...
/**
 * Theme Boost Union Dhoch3 - Custom Behat rules for the 'Smart menus' settings
 *
...
/**
 * Class behat_theme_boost_union_dhoch3_base_smartmenus
 *
 * @package    theme_boost_union_dhoch3
 * @copyright  2023 bdecent GmbH <https://bdecent.de>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class behat_theme_boost_union_dhoch3_base_smartmenus extends behat_base {
    /**
    ...
    public function i_navigate_to_smartmenus() {
        $this->execute('behat_navigation::i_navigate_to_in_site_administration',
            ['Appearance > Boost Union Dhoch3 > Smart menus']);
    }
    ...
    public function i_should_see_smartmenu_in_location($menu, $locations) {
        ...
        if (in_array('Bottom', $locations)) {
            ...
            $this->execute('behat_general::assert_element_contains_text', [$menu, '.boost-union-dhoch3-bottom-menu', 'css_element']);
            $this->execute('behat_general::i_change_window_size_to', ['viewport', 'large']);
        }
        if (in_array('Menu', $locations)) {
            $this->execute('behat_general::assert_element_contains_text', [$menu, '.boost-union-dhoch3-menubar', 'css_element']);
        }
    }
    ...
    public function i_should_see_smartmenu_item_in_location($menu, $item, $locations) {
        ...
        if (in_array('Menu', $locations)) {
            ...
            $this->execute('behat_general::assert_element_contains_text', [$item, '.boost-union-dhoch3-menubar', 'css_element']);
        }
    }
    ...
    public function i_should_not_see_smartmenu_in_location($menu, $location) {
        ...
        if (in_array('Bottom', $locations)) {
            ...
            try {
                $this->find('css_element', '.boost-union-dhoch3-bottom-menu');
            ...
            if ($bottommenufound == true) {
                $this->execute('behat_general::assert_element_not_contains_text',
                        [$menu, '.boost-union-dhoch3-bottom-menu', 'css_element']);
            }
            $this->execute('behat_general::i_change_window_size_to', ['viewport', 'large']);
        }
        if (in_array('Menu', $locations)) {
            ...
            try {
                $this->find('css_element', '.boost-union-dhoch3-menubar');
                $menubarfound = true;
            } catch (ElementNotFoundException $e) {
            ...
            if ($menubarfound == true) {
                $this->execute('behat_general::assert_element_not_contains_text', [$menu, '.boost-union-dhoch3-menubar', 'css_element']);
            }
        }
    }
    ...
    public function i_should_not_see_smartmenu_item_in_location($menu, $item, $location) {
        ...
        if (in_array('Bottom', $locations)) {
            $this->execute('behat_general::i_change_window_size_to', ['viewport', '740x900']);
            $this->execute('behat_general::i_click_on_in_the', [$menu, 'link', '.bottom-navigation', 'css_element']);
            $this->execute('behat_general::assert_element_not_contains_text', [$item, '.boost-union-dhoch3-bottom-menu', 'css_element']);
            $this->execute('behat_general::i_change_window_size_to', ['viewport', 'large']);
        }
        if (in_array('Menu', $locations)) {
            $this->execute('behat_general::i_click_on_in_the', [$menu, 'link', 'nav.menubar', 'css_element']);
            $this->execute('behat_general::assert_element_not_contains_text', [$item, '.boost-union-dhoch3-menubar', 'css_element']);
        }
    }
}
