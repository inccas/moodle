@theme @theme_boost_union_dhoch3 @theme_boost_union_dhoch3_looksettings @theme_boost_union_dhoch3_looksettings_dashboardmycourses
Feature: Configuring the theme_boost_union_dhoch3 plugin for the "Dashboard/My courses" tab on the "Look" page
  ...
  Scenario Outline: Setting: Show course images - Display the course completion progress in the myoverview block or not
    Given the following config values are set as admin:
      | config                           | value          | plugin            |
      | courseoverviewshowcourseprogress | <settingvalue> | theme_boost_union_dhoch3 |
  ...
  Scenario Outline: Setting: Show course images - Display the course image in the myoverview block or not
    Given the following config values are set as admin:
      | config                         | value          | plugin            |
      | courseoverviewshowcourseimages | <settingvalue> | theme_boost_union_dhoch3 |
    And the theme cache is purged and the theme is reloaded
    When I log in as "student1"
    And I follow "My courses"
